...
describe('applySelectionFreeze', () => {
  ...
  it('does NOT restore cached rects when current selection differs from cached id', () => {
    const cache = createSelectionFreezeCache();
    applySelectionFreeze({
      overlayRects: [selectionRect('id-1')],
      currentSelectionId: 'id-1',
    ...
    // User clicked a different element mid-write — must not paint id-1's
    // stale rect over the new target.
    const next: OverlayRect[] = [];
    applySelectionFreeze({
      overlayRects: next,
