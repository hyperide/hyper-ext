...
import type { OverlayRect, OverlayState, OverlayRendererOptions } from './types';
	...
	for (const rect of rects) {
		...
		if (!element) {
			...
			element.style.border =
				rect.type === 'hover' ? HOVER_BORDER : SELECTION_BORDER;
...
function computeOverlayRects(
	...
	if (hoveredId) {
		const isExactItemSelected =
			...
			selectedItemIndices?.get(hoveredId) === hoveredItemIndex;
		...
		if (!isExactItemSelected) {
			...
			if (
				hoveredItemIndex !== null &&
				allHoverElements[hoveredItemIndex]
			) {
			...
			if (hoverElement) {
				...
				const key =
					hoveredItemIndex !== null
						? `hover-${hoveredId}-${hoveredItemIndex}`
						: `hover-${hoveredId}`;
