...
export function chooseIndicatorOrientation(el: HTMLElement, deps?: OrientationDeps): LayoutOrientation {
  ...
  // Cap walk-up at body to avoid scanning past the document. We don't need
  // strict bounds beyond that — flex/grid containers in real layouts sit
  // within a couple of wrappers of any source-bearing element.
 ...
 * `"minmax(0, 1fr) minmax(0, 1fr)"`). `none` or empty means the grid only
 * has implicit columns from `grid-auto-columns` and is treated as
 * single-track.
 */
