...
describe('arbitrary order tokens (review fix)', () => {
  it('bails when any sibling carries a non-int order-[<expr>] at active bp', () => {
    // Plain dense renumber would clobber `order-[var(--idx)]` → `order-2`,
    // destroying the user's CSS-var reference. Falling back to AST move keeps
    // the arbitrary token intact.
    const siblings: SiblingInfo[] = [
      { elementId: 'a:1:1', filePath: 'a.tsx', className: 'order-1', domIndex: 0 },
      { elementId: 'b:1:1', filePath: 'a.tsx', className: 'order-[var(--idx)]', domIndex: 1 },
      { elementId: 'c:1:1', filePath: 'a.tsx', className: 'order-3', domIndex: 2 },
    ];
    expect(
      computeOrderWritePlan({
  ...
  it('bails on non-numeric arbitrary like order-[10rem]', () => {
    const siblings: SiblingInfo[] = [
      { elementId: 'a:1:1', filePath: 'a.tsx', className: 'order-1', domIndex: 0 },
      { elementId: 'b:1:1', filePath: 'a.tsx', className: 'order-[10rem]', domIndex: 1 },
    ];
    expect(
      computeOrderWritePlan({
        siblings,
  ...
  it('does NOT bail on numeric arbitrary order-[<int>] — those round-trip cleanly', () => {
    const siblings: SiblingInfo[] = [
      { elementId: 'a:1:1', filePath: 'a.tsx', className: 'order-1', domIndex: 0 },
      { elementId: 'b:1:1', filePath: 'a.tsx', className: 'order-[5]', domIndex: 1 },
      { elementId: 'c:1:1', filePath: 'a.tsx', className: 'order-3', domIndex: 2 },
    ];
    // Visual sort by numeric: a(1), c(3), b(5). Move a after b → b, a. Renumber.
    const plan = computeOrderWritePlan({
      siblings,
      source: 'a:1:1',
      target: 'b:1:1',
  ...
  it('bail check is bp-scoped: arbitrary at base does not block writes at md', () => {
    // At md viewport, active bp = md. Sibling has `order-[var(--x)]` at base
    // (untouched by md writes) and `md:order-3` at md (parseable). Should NOT bail.
    const siblings: SiblingInfo[] = [
      { elementId: 'a:1:1', filePath: 'a.tsx', className: 'order-[var(--x)] md:order-1', domIndex: 0 },
      { elementId: 'b:1:1', filePath: 'a.tsx', className: 'md:order-3', domIndex: 1 },
    ];
    const plan = computeOrderWritePlan({
      siblings,
      source: 'a:1:1',
      target: 'b:1:1',
      position: 'after',
      viewportWidth: 800, // >= md threshold
    });
    expect(plan).not.toBeNull();
    expect(plan?.breakpoint).toBe('md');
  });
