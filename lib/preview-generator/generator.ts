...
export function generatePreviewContent(entries: PreviewComponentEntry[], options?: GeneratePreviewOptions): string {
  ...
  //     Iterate ALL entries (not just registryEntries): the most useful case is
  //     UI primitives that were filtered OUT of the registry because no sample
  //     could be synthesized — those are exactly the ones that hit the fallback
  //     branch in the iframe and need the detected-exports list to display.
