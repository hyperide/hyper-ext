...
export function generateDemoScriptContent(optionsOrAnalysis: DemoGeneratorOptions | ComponentAnalysis): string {
  ...
  const lines: string[] = [
    ...
    `            data-testid={\`demo-variant-\${variant.id}\`}`,
    `          >`,
    `            {variant.name}`,
    `          </button>`,
    `        ))}`,
...
export function generateDemoE2ETest(analysis: ComponentAnalysis): string {
  ...
  const lines: string[] = [
    ...
    `    await page.waitForSelector('[data-testid="demo-play-pause"]');`,
    ``,
    `    // Check initial state`,
    ...
    `    await page.waitForLoadState('networkidle');`,
    ``,
    `    const playPauseBtn = page.locator('[data-testid="demo-play-pause"]');`,
    ``,
    `    // Pause`,
    ...
    config.variants.length > 1
      ? [
          ...
          `    const variantBtn = page.locator('[data-testid="demo-variant-${config.variants[1]}"]');`,
    ...
    ``,
    `    // Pause auto-play`,
    `    await page.locator('[data-testid="demo-play-pause"]').click();`,
    ``,
    ...config.variants.map((variantId) =>
      [
        `    // Variant: ${variantId}`,
        `    await page.locator('[data-testid="demo-variant-${variantId}"]').click();`,
