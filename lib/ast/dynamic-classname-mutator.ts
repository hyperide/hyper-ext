 ...
 */
export function detectClassNameType(
	element: t.JSXElement,
): 'string' | 'template' | 'call' | 'expression' {
...
export function modifyStringLiteralInPlace(
	...
	const newValue = [preserved, newClassString]
		.filter(Boolean)
		.join(' ')
		.trim();
...
function findStringLiteralByCodeLine(
	...
	if (matchingLineNumbers.length === 0) {
		for (let i = 0; i < sourceLines.length; i++) {
			...
			if (
				normalizedLine === normalizedCodeLine ||
				sourceLines[i].includes(literalValue)
			) {
...
export function modifyByLocations(
	...
	const groupedLocations = new Map<string, {
 ...
 */
function removeConflictingClassesFromString(
	classes: string,
	prefixes: string[],
): string {
 ...
 */
export function wrapInConcatenation(
	element: t.JSXElement,
	newClasses: Record<string, string>,
): void {
	const attr = getAttribute(element, 'className');
	...
	const newExpr = t.binaryExpression(
		'+',
...
export function modifyStaticClassName(
	...
	const preservedClasses = removeConflictingClassesFromString(
		existingClassName,
		prefixes,
	);
	...
	const finalClassName = [preservedClasses, newClassString]
		.filter(Boolean)
		.join(' ')
		.trim();
