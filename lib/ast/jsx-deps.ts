 ...
 * when possible (avoids `import { A } from './x'; import { B } from './x'`).
 *
 ...
 *
 * Returns true if an import was added or merged in (i.e. the localName became
 * importable in target). Returns false if it was already there.
 */
export function replicateImport(
  ...
  // Already imported under the same local name? Skip.
  if (findImportForName(targetAst, localName)) return false;

  const newSourceValue = rewriteImportSource(info.declaration.source.value, sourceFilePath, targetFilePath);

  // Try to merge into an existing import from the same source.
  const existing = targetAst.program.body.find(
    (n): n is t.ImportDeclaration => t.isImportDeclaration(n) && n.source.value === newSourceValue,
  );
  ...
  if (existing) {
    // Default + namespace specifiers must come first; named ones after.
    if (t.isImportDefaultSpecifier(fresh) || t.isImportNamespaceSpecifier(fresh)) {
...
function cloneSpecifier(
  ...
  if (t.isImportSpecifier(spec)) {
    ...
    return t.importSpecifier(t.identifier(spec.local.name), t.identifier(importedName));
  }
