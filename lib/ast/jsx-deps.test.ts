...
describe('replicateImport — illegal merge combinations emit fresh declaration', () => {
  // ES module imports forbid certain specifier combinations on a single
  // declaration. Emitting them anyway produces files that fail to reparse
  // — silently corrupting the target file. Each case below verifies the
  ...
  it('default + default from same module → emits separate declarations', () => {
    const sourceAst = parseCode("import Foo from './x';\nconst _ = <Foo />;\n");
    const targetAst = parseCode("import Bar from './x';\nconst _ = <Bar />;\n");
    const info = findImportInfo(sourceAst, 'Foo');
    const result = replicateImport(targetAst, info, '/w/src/A.tsx', '/w/src/B.tsx');
    expect(result.kind).toBe('added');
    const printed = printAST(targetAst);
    expect(reparses(printed)).toBe(true);
    expect(printed).not.toMatch(/import Foo, Bar from/);
    expect(printed).not.toMatch(/import Bar, Foo from/);
  });

  it('namespace + named on same module → emits separate declarations', () => {
    const sourceAst = parseCode("import * as ns from './x';\nconst _ = <ns.A />;\n");
    const targetAst = parseCode("import { Foo } from './x';\nconst _ = <Foo />;\n");
    const info = findImportInfo(sourceAst, 'ns');
    const result = replicateImport(targetAst, info, '/w/src/A.tsx', '/w/src/B.tsx');
    expect(result.kind).toBe('added');
    const printed = printAST(targetAst);
    expect(reparses(printed)).toBe(true);
    expect(printed).not.toMatch(/import \* as ns, \{/);
  });

  it('named + namespace on same module → emits separate declarations', () => {
    const sourceAst = parseCode("import { Foo } from './x';\nconst _ = <Foo />;\n");
    const targetAst = parseCode("import * as ns from './x';\nconst _ = <ns.A />;\n");
    const info = findImportInfo(sourceAst, 'Foo');
    const result = replicateImport(targetAst, info, '/w/src/A.tsx', '/w/src/B.tsx');
    expect(result.kind).toBe('added');
    const printed = printAST(targetAst);
    expect(reparses(printed)).toBe(true);
    expect(printed).not.toMatch(/\{ Foo \},?\s*\* as ns/);
    expect(printed).not.toMatch(/\* as ns,\s*\{ Foo \}/);
  });

  it('default + named (legal) → merges with default first', () => {
    const sourceAst = parseCode("import Foo from './x';\nconst _ = <Foo />;\n");
    const targetAst = parseCode("import { Bar } from './x';\nconst _ = <Bar />;\n");
    const info = findImportInfo(sourceAst, 'Foo');
    const result = replicateImport(targetAst, info, '/w/src/A.tsx', '/w/src/B.tsx');
    ...
    const printed = printAST(targetAst);
    expect(reparses(printed)).toBe(true);
    expect(printed).toMatch(/import Foo,\s*\{\s*Bar\s*\} from/);
  });

  it('namespace + default (legal) → merges with default first', () => {
    const sourceAst = parseCode("import Bar from './x';\nconst _ = <Bar />;\n");
    const targetAst = parseCode("import * as ns from './x';\nconst _ = <ns.A />;\n");
    const info = findImportInfo(sourceAst, 'Bar');
    const result = replicateImport(targetAst, info, '/w/src/A.tsx', '/w/src/B.tsx');
    expect(result.kind).toBe('added');
    const printed = printAST(targetAst);
    expect(reparses(printed)).toBe(true);
    expect(printed).toMatch(/import Bar,\s*\* as ns from/);
  });
});
