...
- [x] Run the e2e test with fresh extension.
- [x] If GREEN: done, proceed to Task 6.
- [x] If RED: enable verbose logging in the scroll message handlers. Capture logs. Find the gap.
...
- [x] Rebuild and retest until GREEN.

FIX: Added `scrollIntoViewCenterSmooth` call in `hypercanvas:stateUpdate` handler when `selectedIds`
changes (new first ID differs from previous). Uses `findElementsByRef` + itemIndex from
`selectedItemIndices`. Built v0.1.41-b8fix, E2E test B8 PASSED (7.4s). Commit `43b2de52`.
...
- [x] Send summary: what was verified/fixed, test command + result.
- [x] Send screenshot showing the element tree + visible scroll behavior if possible.
