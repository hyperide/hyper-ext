...
- [x] Build extension: `/ext` skill
- [x] Run targeted shard `--grep "logs panel opens after dev server stop"` (run-20260513-205535-45402)
- [x] Confirm `logs panel opens after dev server stop` completes in <400s AND passes
  - Result: 23/25 passed in 6-14s; 2/25 failed on `bulka-the-dog` startup (pre-existing 504 flakiness, unrelated to this fix)
- [x] Check `[fixture-timing]` lines in docker.log — teardown now completes in <1s (previously 10 minutes)
- [ ] Confirm the 5 previously-cascading tests (ET fiber-based selection, nested components, ExportNamedDeclaration traversal, duplicate element preserves integrity, insert element) all pass — deferred to next full shard run
- [x] Send TG report: test time before/after, cascade victim status
