...
- [x] `tg --photo <path> "caption"` with critical visual review — отправлено через `tg --photo <path> "caption"` (универсальный `tg`). До и после-кадры ушли парой с подписями, плюс `tg "..."` с разбором фикса. Визуальная проверка: содержимое viewport визуально другое — scroll отработал, цепочка StateHub.broadcast → PreviewPanel webview → iframe `hypercanvas:scrollToElement` → `scrollIntoViewCenterSmooth` зелёная end-to-end.
