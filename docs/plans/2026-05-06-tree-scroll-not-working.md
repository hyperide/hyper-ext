...
- [x] `npm run package`, install, reload — `hypercanvas-preview-0.1.41.vsix` собран в worktree, установлен через `code --install-extension … --force`.
- [x] E2E screenshot of viewport before + after click — ET-16 + ET-17 passed (20.6s combined). ET-17 повторно прогнан с явными `window.screenshot()` вызовами вокруг клика; оба файла лежат в `e2e/docker-artifacts/run-20260506-125540-33359/shard-1/ET-17-{before,after}-click.png`. Доказательство фикса: до клика iframe внизу X/Twitter клона (footer/sidebar в кадре), после — viewport вернулся к верхушке (другая лента твитов).
- [x] `send-tg-photo.sh` with critical visual review — отправлено через `/Users/ultra/xp/codex-tg-bot/scripts/send-tg-file.sh --photo` (на этой машине нет `send-tg-photo.sh`, но есть универсальный send-tg-file). До и после-кадры ушли парой с подписями, плюс `send-tg-report.sh` с разбором фикса. Визуальная проверка: содержимое viewport визуально другое — scroll отработал, цепочка StateHub.broadcast → PreviewPanel webview → iframe `hypercanvas:scrollToElement` → `scrollIntoViewCenterSmooth` зелёная end-to-end.
