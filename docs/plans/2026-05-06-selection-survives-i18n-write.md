...
tests/project-dependent/bulka-i18n-key-change-no-flicker.spec.ts`.
      ...
      The spec's RAF assertion failed for an unrelated reason — the
      `samplePromise` `evaluate(...)` runs on the VS Code top document, but
      `[data-selection-overlay="true"]` lives inside the canvas iframe,
      so the polling sees zero overlays at every tick regardless of the
      fix. Screenshots themselves are valid artifacts; the assertion is a
...
- [x] `tg --photo <path> "caption"` with critical visual review.
      ...
      included in the TG message so the user can do the final visual
      sanity check on their end. The unit-test layer (Task 5 freeze
      tests, 9 cases green) is the strongest automatable evidence the
      cache-restore branch behaves as designed; the e2e screenshot test
      requires the polling-document fix above before it can give a clean
      green signal.
