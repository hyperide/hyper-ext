...
- [ ] Fix: still needed. After 5 build+test iterations with
      progressively deeper instrumentation, observed:
      `[drag-handler-loaded]` logs twice per test (iframe loads at
      same href ~1s apart, suggesting Vite HMR reload or initial-
      load reload), `[drag-up]` (pointerup) fires 8 times, but
      `[drag-evt]` (pointerdown), `[drag-window-pd]` (window-level
      pointerdown), `[drag-mousedown]` (document mousedown) fire
      ZERO times for the img drag. That's not normal browser
      semantics. Two leading hypotheses (per advisor consult):
        (a) Most likely: 8 [drag-up] events are setup/teardown
            clicks NOT the test's mouse.up. Test's actual
            mouse.down/up never reach this iframe — coordinate
            targeting issue (img is 16x16, may miss; iframe
            position may shift). Discriminator unfinished — broke
            on `appFrame.evaluate is not a function` (appFrame is
            a Locator not a Frame; need
            `appFrame.locator(...).evaluate(...)` or
            `await canvas.getActualFrame()`).
        (b) Less likely: Chromium quirk — pointerup without
            pointerdown for img drag specifically.
      Next iteration must: fix the discriminator (use a real
      Frame, not the Locator that `getAppFrame()` returns), then
      bracket the `dragInIframe` call with `[test:drag-START]` /
      `[test:drag-END]` markers, then confirm whether the actual
      drag's pointerdown/up reach the iframe at all. If they don't,
      bug is in coordinate targeting / iframe positioning, not in
      the drag handler chain. If pointerdown DOES fire but the
      logs are filtered out, the resolution is on the iframe-side
      console wrapper.
