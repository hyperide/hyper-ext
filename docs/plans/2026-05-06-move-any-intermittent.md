...
- [ ] Reproduce the failure deterministically. PI-5-DR-EK-IMG
      (fresh state) is the cleanest repro. Current run-20260507-120925
      shows zero AstBridge calls during this test. Confirm by running
      the test alone and grepping the new run's ast-debug.log:
      `bun run test:docker -- -g "PI-5-DR-EK-IMG"` then
      `grep moveElement docker-artifacts/run-*/shard-*/ast-debug.log`.
- [ ] Trace the iframe drag handler for img-source mouse-down. Suspect
      that `_dragPointerDown` (or the resize-handles overlay) returns
      early for img elements because the resize handles intercept the
      mouse event. Check `vscode-extension/hypercanvas-preview/src/
      iframe/iframe-interaction.ts` (or whichever file owns
      `_dragPointerDown` / pointer-down dispatch) for an early return
      on `tagName === 'IMG'` or for `pointerEvents` shenanigans on
      the resize-handle overlay.
- [ ] Fix: img must be a valid drag source. Either disable the
      resize-handle overlay's pointerEvents during the initial
      mouse-down → first-move window, or attach the drag listener on
      the same element that owns the resize handles so the gesture
      isn't lost.
