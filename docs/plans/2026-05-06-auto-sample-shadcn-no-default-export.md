...
- [x] `npm run package`, install, reload. (skipped — not automatable in
      ...
      which is a worktree-state issue, not a code issue. The user runs
      this step via `/ext` after merging.)
- [x] Open carousel.tsx in Hyper Canvas, screenshot proof of render.
      (skipped — covered by the E2E spec added in Task 5
      `../ext-test-projects/e2e/tests/project-dependent/component-load.spec.ts`,
      which opens `bulka-the-dog/client/components/ui/carousel.tsx` via
      `setupPreviewWithDevServer` — same code path the user takes —
      asserts the iframe renders something other than "Loading…" /
      "No sample for this component" / "Component not found", and writes
      `carousel-auto-sample-preview.png` to the test artifact dir.
