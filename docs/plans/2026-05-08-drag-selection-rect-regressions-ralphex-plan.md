...
- [x] E2E screenshots from Tasks 1+2, manually inspected (CLAUDE.md screenshot rule: rect
      ...
      ("Виден ли элемент / состояние, демонстрирующее что баг решён?" — НЕТ),
      so deliberately NOT sent to TG as fix-proof. Will be re-run unmodified
      once the harness regression ticket lands.
