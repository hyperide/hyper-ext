...
- [ ] Pass `jsescOption: { minimal: true }` (or equivalent) to the babel-generator
  call inside `writeTsLocaleValue` so freshly-emitted string literals keep
  their original code points.
- [ ] Verify `retainLines: true` still works — line numbers must not drift.
- [ ] Re-run the 3 RED tests added in Task 1; all must turn GREEN.
- [ ] Re-run the full `shared/i18n-text/__tests__` suite and confirm no regressions.
