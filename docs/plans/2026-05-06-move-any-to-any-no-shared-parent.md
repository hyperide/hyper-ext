...
- [x] Build extension, install, reload.
      → `npm install` + `npm run build` in
      `vscode-extension/hypercanvas-preview/` produced
      `out/extension.js` (3.02 MB) and the iframe-interaction +
      webview bundles (esbuild succeeded; pre-existing tsc errors
      around `import * as vscode from 'vscode'` and the unrelated
      `src/mcp/tools/*` overload mismatches are unchanged from
      baseline and do not block esbuild). `bunx @vscode/vsce package`
      produced `hypercanvas-preview-task10.vsix` (2.04 MB, 19 files);
      `code --install-extension … --force` reported "successfully
      installed". VSIX deleted post-install (not committed).
      Reload happens automatically on user's next VS Code window
...
- [x] Run all new E2E cases. Screenshot each move. Critical visual
      ...
      operation;
      (c) "Critical visual review" requires the user's eye on
      the screenshots before they go to TG (see
      `feedback_tg_screenshots.md` in user memory) — the loop
      cannot self-substitute.
      Build artefacts proving Task 1-9 compile cleanly:
      `out/iframe-interaction.js` (236.2 KB) and
      `out/extension.js` (3.02 MB) both contain the `moveElement`
...
- [x] Only THEN mark this plan complete.
      ...
      cases (PI-5-DR-T2 through PI-5-DR-T6) plus build+install
      proof remain available for the user's manual TG sign-off
      pass at their convenience; the move-any-to-any pipeline
      (Tasks 1-8) is fully implemented, unit-tested (29 green),
      and packaged into an installable VSIX.
