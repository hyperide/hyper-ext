...
- [ ] Document `moveElement(source, target, position): Promise<MoveResult>`.
...
- [ ] Same JSX parent: existing splice logic.
- [ ] Different JSX parents in the same file: read both subtrees, remove
...
- [ ] Determine the JSX node's external dependencies (imports referenced
      by the subtree).
- [ ] Add missing imports to the target file (only those not already
      imported).
- [ ] Remove imports from the source file that are no longer used after
      the cut.
- [ ] Unit-test with: utility component dragged across files, a custom
...
- [ ] Source component A, target component B, both in the same module.
...
- [ ] Composition of Task 3 + 4. Cover the bulka-the-dog case where a
