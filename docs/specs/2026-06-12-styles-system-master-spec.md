...
**Status:** Rev 0.3.5 (descriptive status-accuracy correction of Rev 0.3.4, no re-ratification) — **OD-1..OD-5 RATIFIED by the CTO** (2026-06-14, [HYP-722](https://linear.app/glide-vc/issue/HYP-722)) + **Rev-0.3 CTO additions ratified** (`collateral-broken` 6th state, [§8.4-bis](#84-bis-error-rollback--recovery-ux-ux-is-important-everywhere) error/rollback UX, `sourceHash` identity, `ParsedStyles`-DELETE reconciliation — see the revision table); OD-9 now CLOSED (merged), OD-6/7/8/10/11 still open (see [§0.1](#01-title-status-ownership-revision-table)). Prose + visual assets present. **AS-IS re-anchor (0.3.1/0.3.2), corrected in 0.3.3, refreshed in 0.3.4/0.3.5:** the **B0 write-transaction foundation has SHIPPED** (`lib/style-write/transaction/`, both realms); **B1 runtime-verify has a first ext-side verify-and-retry SLICE shipped** ([HYP-987](https://linear.app/glide-vc/issue/HYP-987) M1) — [HYP-991](https://linear.app/glide-vc/issue/HYP-991) (#666) **merged** 2026-07-17, [HYP-990](https://linear.app/glide-vc/issue/HYP-990) M2 (#665) **MERGED** 2026-08-18 (`d386cc43`) — but the shared `lib/style-write/runtime-verify/` foundation (dual-settle, fail-closed matrix, both realms) is **still absent** (D19). **A1 forward-detector ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)):** an implementation exists on [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719) (`lib/style-read/forward-detect*.ts`) but is **NOT merged** — Codex review left 1 unaddressed P1 (the detector is wired into the VS Code RPC service only; the SaaS/browser read path in `client/lib/platform/hooks/useElementStyleData.ts` does not call it) plus 4 P2s (declared-prop treated as forwarding proof, unhandled multi-return-path components, unverified Slot branches, transformed-binding false exclusions) — so **on `main`, A1 is still unbuilt** and `StyleReadService.buildElementFacts:763` still hardcodes `acceptsClassName:true`; see revision row 0.3.4 and [§3.15](#315-as-is-subsystem-status-roll-up). **Spec ticket:** [HYP-722](https://linear.app/glide-vc/issue/HYP-722) · **Program ticket:** [HYP-299](https://linear.app/glide-vc/issue/HYP-299). **Owner:** A. Ultra (CTO) + styles committee.
...
| VS Code ElementFacts (sourceOwners/propMappers/themeCaps/computedStyle)        | **PARTIAL**                                    | hardcoded empty/`true`; `buildElementFacts:763`; A1 replacement implemented but unmerged ([HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719) — 1 open P1 + 4 open P2, see [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
home is [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)"), and the [Part 14.2](#142-phase-map-with-the-live-tickets) Phase-3 row all cite [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home); no other section re-specifies the detector. (**On `main` A1 is still unbuilt** —
`StyleReadService.buildElementFacts:763` hardcodes `acceptsClassName:true`, the fact A1 replaces; D3/D19.)

An implementation exists on [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719)
(`lib/style-read/forward-detect.ts` + `forward-detect-type.ts` + `forward-detect-trace.ts`, matching the
target module below) but is not merged: Codex review left an unaddressed **P1** — the detector is invoked
only from the VS Code RPC service (`StyleReadService.ts`); the SaaS/browser read path
(`client/lib/platform/hooks/useElementStyleData.ts`) still reads canvas-engine/DOM facts and never calls
it, so a merge as-is would leave the SaaS realm exactly as blind as before — plus 4 unaddressed P2s:
declared-but-unforwarded prop types treated as forwarding proof; a transformed-yet-still-referenced
destructured binding wrongly emitted as a high-confidence negative exclusion; only the last JSX-like
`return` is inspected, missing earlier conditional-return paths; and any `Slot`-importing component is
assumed to forward both channels without verifying either branch does. (Verified 2026-08-18 via
`gh pr view 719` / `gh api .../pulls/719/comments`.) **Merging #719 with the P1 fixed is necessary but
not sufficient to unblock Phase 3 on its own** — the [Part 14.2](#142-phase-map-with-the-live-tickets) table's `A1 → B1` hard dependency still applies, and the full B1 foundation
...
VS Code **extension** realm, which has zero wiring of this ladder — starved until the A1 forward-detector
lands per its dependency edge ([Part 14.2](#142-phase-map-with-the-live-tickets); canonical home is [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)) — ([HYP-427](https://linear.app/glide-vc/issue/HYP-427), Backlog), and (b) production
...
>
> **A1 forward-detector (Phase 3):** an implementation now exists on [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229)
> PR [#719](https://github.com/hyperide/hyper-saas/pull/719) (`lib/style-read/forward-detect*.ts`) but is
> **NOT merged** — Codex review left 1 unaddressed P1 (wired into the VS Code RPC service only; the
> SaaS/browser read path in `client/lib/platform/hooks/useElementStyleData.ts` never calls it) plus
> 4 P2s (declared-prop-as-proof, unhandled multi-return-path, unverified Slot branches, a
> transformed-binding false exclusion — full list in revision row 0.3.4). **On `main`, A1 remains
> unbuilt** ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)). Merging #719 with the P1
> resolved is necessary but **not sufficient** to unblock Phase 3: the table's `A1 → B1` hard
> dependency still applies, and the full B1 foundation is itself still PLANNED (Phase-2 row above).
...
| **3 — AI ladder + Tier-2 source resolution + color UI**                    | A1 forward-detector (real per-channel forwarding facts, [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)); Tiers 0-5 AI/deterministic ladder (AI routes/ranks, probe is the gate); host-side Tier-2 CSS write-target resolution; static cva-variant resolver; **the project-independent ColorCombobox composition test that picks a color and asserts the source edit landed (D30)** | [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) (implemented on PR [#719](https://github.com/hyperide/hyper-saas/pull/719), NOT merged — 1 open P1 + 4 open P2, dep B1, [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)), [HYP-704](https://linear.app/glide-vc/issue/HYP-704) (CSS/var/module exact-only + floor), [HYP-705](https://linear.app/glide-vc/issue/HYP-705) (cva resolver), [HYP-686](https://linear.app/glide-vc/issue/HYP-686) (intent→token, later), [HYP-349](https://linear.app/glide-vc/issue/HYP-349)-followup (color composition test, D30)                                                                                                 | A1 → B1; [HYP-704](https://linear.app/glide-vc/issue/HYP-704)/705 → A1 + B1 (DO NOT start 704/705 before B0/B1 exist — discovery-tickets §[HYP-544](https://linear.app/glide-vc/issue/HYP-544)); D30 test → B1 (asserts landing)                                                                                                                                                                                                                                                                                                                             | 7, 9.2a, 10, 12.4, 12.5 |
