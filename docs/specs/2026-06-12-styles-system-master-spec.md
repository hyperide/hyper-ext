...
**Status:** Rev 0.3.8 (closed follow-up ticket, no re-ratification) — **OD-1..OD-5 RATIFIED by the CTO** (2026-06-14, [HYP-722](https://linear.app/glide-vc/issue/HYP-722)) + **Rev-0.3 CTO additions ratified** (`collateral-broken` 6th state, [§8.4-bis](#84-bis-error-rollback--recovery-ux-ux-is-important-everywhere) error/rollback UX, `sourceHash` identity, `ParsedStyles`-DELETE reconciliation — see the revision table); OD-9 now CLOSED (merged), OD-6/7/8/10/11 still open (see [§0.1](#01-title-status-ownership-revision-table)). Prose + visual assets present. **AS-IS re-anchor (0.3.1/0.3.2), corrected in 0.3.3, refreshed in 0.3.4/0.3.5/0.3.6/0.3.7/0.3.8:** the **B0 write-transaction foundation has SHIPPED** (`lib/style-write/transaction/`, both realms); **B1 runtime-verify has a first ext-side verify-and-retry SLICE shipped** ([HYP-987](https://linear.app/glide-vc/issue/HYP-987) M1) — [HYP-991](https://linear.app/glide-vc/issue/HYP-991) (#666) **merged** 2026-07-17, [HYP-990](https://linear.app/glide-vc/issue/HYP-990) M2 (#665) **MERGED** 2026-08-18 (`d386cc43`) — but the shared `lib/style-write/runtime-verify/` foundation (dual-settle, fail-closed matrix, both realms) is **still absent** (D19). **A1 forward-detector ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)):** [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719) (`lib/style-read/forward-detect*.ts`) **MERGED to `main` 2026-08-18** (`860f66c1`) — `StyleReadService.buildElementFacts` now calls the real detector on the VS Code side; `acceptsClassName:true` is no longer hardcoded there. The PR's one open P1 (SaaS/browser read path in `client/lib/platform/hooks/useElementStyleData.ts` never called the detector) was closed via [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280) — see revision row 0.3.6. The 4 in-detector P2s (declared-prop treated as forwarding proof, unhandled multi-return-path components, unverified Slot branches, transformed-binding false exclusions) remain open. Separately, **[HYP-1235](https://linear.app/glide-vc/issue/HYP-1235) — unifying the write-path's OWN older, coarser check onto this detector — is now CLOSED this revision**: the VS Code extension's write-path pre-check (`style-forwarding-check.ts`) is rewired onto `detectForwarding`; a 3-model review round on the closing PR also caught and fixed a silently-lost monorepo workspace-package resolution capability, now shared via a new `lib/ast/workspace-package-entry.ts` module both this detector and the shared executor's separate HYP-995 write-path check (`lib/style-write/component-forwarding.ts`) call — but that executor check's own PER-PROP forwarding analysis remains unmigrated, tracked as its own follow-up. Separately, **[HYP-1294](https://linear.app/glide-vc/issue/HYP-1294) — giving `componentPropSurface` (fetched since 0.3.6 but never read anywhere) a real consumer — is now CLOSED this revision**: a proactive, dismissible Right Sidebar inspector toast warns the moment the selected element's component provably has no style-write channel at all, before any write is attempted; the VS Code extension's `styles:response` RPC now also serializes `componentPropSurface` (previously SaaS/browser-only), so the same warning fires on both platforms. Deliberately informational only — the D3 stylability ladder stays STAGED and unconsulted by the live write path. See [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home). See [§3.15](#315-as-is-subsystem-status-roll-up). **Spec ticket:** [HYP-722](https://linear.app/glide-vc/issue/HYP-722) · **Program ticket:** [HYP-299](https://linear.app/glide-vc/issue/HYP-299). **Owner:** A. Ultra (CTO) + styles committee.
...
reconciliation — revision-table row 0.3); OD-9 now **CLOSED** (merged, revision-table row 0.3.3); OD-6/7/8/10/11 still open. This document is the **single source of truth** for the
...
| 0.3.3 | 2026-08-14 | styles committee (audit pass, [HYP-722](https://linear.app/glide-vc/issue/HYP-722))                                                        | **Descriptive status-accuracy correction (no re-ratification — per the patch-bump rule below).** A verified audit against live Linear/GitHub/`main` checked six status clusters, found five stale and corrected them in place, and confirmed the sixth already accurate (no structural/prescriptive changes): **(A)** multi-select style write ([HYP-271](https://linear.app/glide-vc/issue/HYP-271)/[HYP-581](https://linear.app/glide-vc/issue/HYP-581), PR [#270](https://github.com/hyperide/hyper-saas/pull/270)) — was described as "unmerged branch, starved"; PR #270 in fact **merged to `main` 2026-06-18** (`4c243f67`) a month before Rev 0.3.2's own re-anchor date, so the SaaS-realm sub-status is `WORKS`, not `PLANNED` — corrected the composite feature status to **PARTIAL** (SaaS shipped; ext realm still unwired, [HYP-427](https://linear.app/glide-vc/issue/HYP-427) Backlog; frozen-plan dispatch incomplete, [HYP-664](https://linear.app/glide-vc/issue/HYP-664) Backlog) across [§3.9](#39-modes--jsx-vs-dom-single-vs-multi), [§3.15](#315-as-is-subsystem-status-roll-up), [§4.1](#41-speccode-discrepancies-d1-d11) (D7), [§11.1](#111-the-vectorized-model-single-element-is-n1), and [§14.1](#141-why-the-safety-net-comes-first-dependency-reasoning)/[§14.2](#142-phase-map-with-the-live-tickets). **(B)** OD-9 / the shared-PropsEditor PR ([HYP-709](https://linear.app/glide-vc/issue/HYP-709)/[HYP-716](https://linear.app/glide-vc/issue/HYP-716), PR [#453](https://github.com/hyperide/hyper-saas/pull/453)) — was described as unmerged with no ext PropsEditor UI; PR #453 in fact **reached `main`** — its own 2026-06-12 commit (`23ed146c`) was orphaned by a history rewrite and never landed on `origin/main`, but [#593](https://github.com/hyperide/hyper-saas/pull/593) re-landed the same work on 2026-07-01 (`0e27bc12`, confirmed via `git merge-base --is-ancestor`), and the ext webview confirmed (by code inspection, not just the compiled `out/` artifact) to render `<PropsEditor/>` via the platform-converged `usePropsEditorSource` seam. The actual `main` landing (2026-07-01) postdates Rev 0.3 (2026-06-14) — so the "unmerged" framing was accurate when Rev 0.3 shipped; the staleness set in afterward, since Rev 0.3.2 (2026-07-17, itself after the 2026-07-01 landing) still carried the outdated status forward instead of picking it up — corrected to **CLOSED/resolved** across [§4.4](#44-intentspec-tensions-d24-d29) (D29), [§6.5](#65-surface-decision--per-property-editability), [§9.8](#98-type-intelligence-lsp--applications--realm-boundary), [§12.2](#122-token-providers--the-project-palette-gap), and [§13.7](#137-od-6-through-od-11--the-second-tier-opens) (OD-9); the pre-existing revision-table rows (0.1-0.3.2) are left as the append-only historical record and are not retroactively edited. **(C)** [HYP-990](https://linear.app/glide-vc/issue/HYP-990)/[HYP-991](https://linear.app/glide-vc/issue/HYP-991) — both were described as "in review"; [HYP-991](https://linear.app/glide-vc/issue/HYP-991)/#666 in fact **merged 2026-07-17**, while [HYP-990](https://linear.app/glide-vc/issue/HYP-990)/#665 remains open and **CI-red** (re-verified 2026-08-14, unchanged since 2026-07-18) with scope grown to include the §9.4 confidence×verifiability matrix — corrected across [§1.2](#12-what-works-today-the-honest-floor), [§3.15](#315-as-is-subsystem-status-roll-up), and [§14.2](#142-phase-map-with-the-live-tickets) (both the AS-IS re-anchor blockquote and the Phase-2 table row). **(D)** SaaS i18n key read ([HYP-372](https://linear.app/glide-vc/issue/HYP-372)) — was described as `BROKEN/MISSING`/`Todo, BLOCKED`; [HYP-372](https://linear.app/glide-vc/issue/HYP-372) is in fact **Done**, shipped via PR #477 (read + retarget) and PR #478 (full OPFS parity) — corrected to **WORKS** across [§3.13](#313-cross-realm-transport-today), [§3.15](#315-as-is-subsystem-status-roll-up), [§4.1](#41-speccode-discrepancies-d1-d11) (D10, now resolved), [§5.4](#54-realm-model--three-first-class-realms-as-transport-rows-over-one-contract), and [§14.4](#144-error--edge-case-state-map); residual hardening now tracked under [HYP-746](https://linear.app/glide-vc/issue/HYP-746) (Backlog). **(E)** [HYP-553](https://linear.app/glide-vc/issue/HYP-553) (per-class Tailwind confidence) — was listed as a pending Phase-2 building ticket; it **merged 2026-06-17** (PR #468) — the [§14.2](#142-phase-map-with-the-live-tickets) Phase-2 row mention is re-annotated as already-merged/shipped-ahead-of-slot rather than pending. **(F)** [HYP-600](https://linear.app/glide-vc/issue/HYP-600)/606-609 (CSS-frameworks build-out) — audited, found accurate, **no change**. No prescriptive (TO-BE) change; OD-1..OD-8/OD-10/OD-11 untouched; OD-9 flips from open to CLOSED as a ratification-executed item, not a re-decision. |
| 0.3.4 | 2026-08-18 | styles committee (audit pass, [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229)/[HYP-990](https://linear.app/glide-vc/issue/HYP-990)) | **Descriptive status-accuracy correction (no re-ratification — per the patch-bump rule below).** Two updates, both re-verified live against GitHub: **(A)** A1 forward-detector ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home), [§14.2](#142-phase-map-with-the-live-tickets) Phase-3) — a real implementation now exists on [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719) (`lib/style-read/forward-detect.ts`, `forward-detect-type.ts`, `forward-detect-trace.ts`), but it is **NOT merged to `main`** — a Codex multi-model review left **1 unaddressed P1** ("Wire forwarding detection into the SaaS read path": the invocation only runs in `vscode-extension/hypercanvas-preview/src/services/StyleReadService.ts`; the browser/SaaS branch in `client/lib/platform/hooks/useElementStyleData.ts` still reads from the canvas engine/DOM and never calls the new detector) plus **4 unaddressed P2s** (a declared-but-unverified prop type is treated as forwarding proof in `forward-detect-type.ts`; `forward-detect-trace.ts` emits a false high-confidence-negative exclusion for a transformed-but-still-referenced destructured binding; `primaryReturnedRoot` in `forward-detect-trace.ts` only inspects the last JSX-like `return`, missing earlier conditional-return paths; `forward-detect.ts` assumes any `Slot`-importing component forwards both channels without verifying either branch actually does). Corrected the top-of-doc status line, the AS-IS re-anchor blockquote, and [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)'s "on `main` A1 is unbuilt" parenthetical to cite the open PR and its review gate instead of silence — merging #719 with the P1 resolved is necessary but **not sufficient** to unblock [§14.2](#142-phase-map-with-the-live-tickets) Phase 3: the table's `A1 → B1` hard dependency still applies and the full B1 foundation is itself still PLANNED (Phase-2 row), independent of A1's status. **(B)** [HYP-990](https://linear.app/glide-vc/issue/HYP-990) M2 (PR [#665](https://github.com/hyperide/hyper-saas/pull/665)) — Rev 0.3.3 (2026-08-14) recorded it "open, CI-red"; re-verified 2026-08-18, **CI is now fully green and GitHub reports the PR MERGEABLE** (a separate in-flight effort fixed the red CI; no GitHub review submitted yet, still not merged) — corrected across the top status line, the AS-IS re-anchor blockquote, and the [§14.2](#142-phase-map-with-the-live-tickets) Phase-2 row. No prescriptive (TO-BE) change; no OD register change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 0.3.5 | 2026-08-18 | styles committee (multi-model review pass, [HYP-722](https://linear.app/glide-vc/issue/HYP-722))                                           | **Descriptive self-consistency correction of Rev 0.3.4 (no re-ratification — per the patch-bump rule below).** A 3-model `review diff` pass (Opus/Fable/k3) on the still-open PR #718 diff found that the 0.3.4 pass itself introduced several internal contradictions and one fresh fact went stale again within hours — all corrected here, none prescriptive: **(A)** [HYP-990](https://linear.app/glide-vc/issue/HYP-990) M2 (#665) **MERGED 2026-08-18** (`d386cc43`) — the fact moved again after 0.3.4 was written ("CI green/MERGEABLE, not yet merged"); corrected across the top status line, [§1.2](#12-what-works-today-the-honest-floor) (which 0.3.4 had missed, leaving it asserting stale "CI-red" while every other updated location said "CI-green" — a direct self-contradiction), [§3.15](#315-as-is-subsystem-status-roll-up), and both [§14.2](#142-phase-map-with-the-live-tickets) locations (blockquote + Phase-2 row). **(B)** The 0.3.3/0.3.4 passes' own corrected narrative — PR #453's 2026-06-12 commit `23ed146c` was orphaned and never reached `main`; [#593](https://github.com/hyperide/hyper-saas/pull/593) landed the work 2026-07-01 via `0e27bc12` — was re-contradicted by newly-touched text in this same diff still saying "merged 2026-06-12" ([§6.5](#65-surface-decision--per-property-editability) intro) and "merged 2026-06-12" ([§9.8](#98-type-intelligence-lsp--applications--realm-boundary)), plus two leftover "unmerged #453" mentions (the D29 width/datalist note and a separate §12.3 blockquote reference); all three corrected to the verified 2026-07-01/#593 landing. **(C)** [§14.1](#141-sequencing-principle)'s new A1 bullet reintroduced the stale `StyleReadService.buildElementFacts:704` line-cite this diff's OTHER new text had already corrected to `:763` (the line moved as the file grew) — fixed to `:763`. **(D)** The i18n `WORKS`/"full parity" language this diff's own [§3.15](#315-as-is-subsystem-status-roll-up) row, [§4.1](#41-speccode-discrepancies-d1-d11) D10, [§5.4](#54-realm-model--three-first-class-realms-as-transport-rows-over-one-contract) transport table, and [§14.4](#144-acceptance-gate--erroredge-case-matrix) row used contradicted the same diff's own [§3.13](#313-cross-realm-transport-today) correction (Docker SaaS existing-key-retarget-only vs. NodePod full parity) — all four re-qualified to **PARTIAL for Docker SaaS, WORKS for NodePod**, matching §3.13's grading convention used elsewhere in this same diff for composite (not blanket) statuses. **(E)** [§14.2](#142-phase-map-with-the-live-tickets)'s Phase-4 dependency-column edit narrowed the ratified `all → A1` hard dependency to an ext-only edge under the "no re-ratification" banner — reverted to `all → A1`; this section is prescriptive (TO-BE) and this patch-bump pass is not authorized to re-scope a dependency edge, only to correct descriptive status prose; the true status fact (HYP-271's SaaS slice shipped independent of A1) is kept as an annotation, not a graph change. **(F)** [§3.9](#39-modes--jsx-vs-dom-single-vs-multi)'s new claim that props-mode (Tamagui/RN) multi-select "reports" the unavailable message was inaccurate — the exclusion is silent (`useStyleSync.ts` drops the write with only `console.warn`; the visible fallback is a read-side check on `multiSelectMerged`, and `TamaguiAdapter.read()` always succeeds, so the normal inspector renders with no visible error) — corrected to describe the actual (silent-drop) behavior and flag it as a live gap. **(G)** D7's claim that `ast:updateStylesBatch` is "v1-MERGED" conflated the merged live SaaS batch ROUTE (`useStyleSync.flushQueue` → `engine.updateASTStylesBatch` → `POST /api/update-component-styles-batch`) with the still-STAGED ext-host MESSAGE of the same-looking name (`batch-style-write-plan.ts`'s own file header) — the same conflation class as one of Rev 0.3.3's original 4 P2 fixes, reintroduced here in a different cell; separated the two. No prescriptive (TO-BE) change beyond the (E) revert (itself a revert TO the ratified state, not a new one); no OD register change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 0.3.6 | 2026-08-19 | styles committee (audit pass, [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229)/[HYP-1280](https://linear.app/glide-vc/issue/HYP-1280)) | **Descriptive status-accuracy correction (no re-ratification — per the patch-bump rule below), plus one closed follow-up ticket.** **(A)** A1 forward-detector ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home), [§14.2](#142-phase-map-with-the-live-tickets) Phase-3) — Rev 0.3.4/0.3.5 recorded [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719) as open/unmerged; re-verified live via `gh pr view 719`, it **MERGED to `main` 2026-08-18** (`860f66c1`). Corrected the top-of-doc status line, this row, [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)'s "on `main` A1 is still unbuilt" parenthetical, [§3.15](#315-as-is-subsystem-status-roll-up)'s ElementFacts roll-up row, and the [§14.2](#142-phase-map-with-the-live-tickets) AS-IS re-anchor blockquote + Phase-3 row to say MERGED — `StyleReadService.buildElementFacts` now calls `detectForwarding` for real on the VS Code side. **The `A1 → B1` hard dependency in [§14.2](#142-phase-map-with-the-live-tickets) is UNCHANGED** — A1 merging does not itself unblock Phase 3; the full B1 foundation (dual-settle, fail-closed matrix, both realms) remains PLANNED per the Phase-2 row, independent of A1's status (same caveat 0.3.4 already stated about "necessary but not sufficient," now simply true instead of conditional). **(B)** The PR's own Codex-review P1 — "wire forwarding detection into the SaaS read path" (the browser/SaaS branch in `client/lib/platform/hooks/useElementStyleData.ts` read canvas-engine/DOM facts directly and never called the new detector, so SaaS still planned every write blind to real forwarding facts while the VS Code extension was already protected) — was filed as its own ticket, [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280), and **CLOSED this revision, scoped precisely to what its own acceptance criteria asked for — fetching and exposing the facts, not yet enforcing them.** A new read-only `GET /api/element-forwarding` server route (`server/routes/readElementForwarding.ts`, reusing the shared `resolveElementTarget` element-resolution helper extracted out of `updateComponentStyles.ts` into `server/lib/resolve-element-target.ts`, and resolving tsconfig path aliases the same way `StyleReadService._loadAliasMap` does on the VS Code side — a 3-model review round on this same revision caught the alias gap: an unaliased server-side lookup would have silently fallen fail-open on the dominant `@/components/Button`-style import, the opposite of the parity this route exists to deliver) calls the real `detectForwarding` and projects it through a newly-shared `projectForwardDetectionToPropSurface` (moved out of `StyleReadService.ts`'s private duplicate into `lib/style-read/forward-detect.ts` so both platforms use one projection, not two). `useElementStyleData.ts`'s browser-mode path fetches it asynchronously (non-blocking — the synchronous engine+DOM read is unaffected, re-fetches on `refreshKey` like its sibling effect, and discards a stale response for an already-abandoned selection) and exposes it as a new `componentPropSurface` field on `ElementStyleData` (browser/SaaS mode only — VS Code does not currently serialize the same facts onto its `styles:response` RPC payload, so the field stays unset there). Verified live: `server/routes/readElementForwarding.test.ts` exercises the REAL detector — including a same-file wrapper that names `className` out before spreading `...rest`, and, separately, a `tsconfig` path-ALIASED cross-file wrapper — and asserts a high-confidence `no-host-forward` exclusion reaches the HTTP response either way; `client/lib/platform/hooks/useElementStyleData.forwarding.test.ts` covers the client fetch helper's request shape, the hook's reset-on-selection-change and stale-response-discard behavior, and — as a composition proof, not a claim about shipped enforcement — pipes the same non-forwarding fact through the existing (STAGED, not yet live anywhere) shared `lib/style-write/stylability-ladder.ts` planner function and confirms it resolves to rung `L3`. **Honest scope note (self-caught in this same revision's review round, before it could become a future correction like 0.3.3/0.3.4 had to make for other claims):** an earlier draft of this row claimed the SaaS browser path now "produces the same pre-write EXCLUSION the VS Code extension already had" — that overstated it. No shipped code (browser or VS Code) currently reads `componentPropSurface` in a live write or UI path; the live SaaS write-time refusal is still provided separately by the older, coarser `lib/style-write/component-forwarding.ts` check ([HYP-995](https://linear.app/glide-vc/issue/HYP-995)), unrelated to this ticket. Wiring `componentPropSurface` into a real consumer (an inspector warning, and/or a future live planner) is tracked as its own follow-up, [HYP-1294](https://linear.app/glide-vc/issue/HYP-1294); unifying the write-time check itself onto the richer A1 detector is the separate, still-open [HYP-1235](https://linear.app/glide-vc/issue/HYP-1235). `ElementForwardDetection` (a byte-for-byte duplicate of `forward-detect.ts`'s own `ForwardDetectionResults`, orphaned once `StyleReadService.ts`'s import moved) was deleted from `lib/style-read/types.ts` in the same pass; `ElementStyleFacts.forwardDetection` now types directly against `ForwardDetectionResults`. No prescriptive (TO-BE) change; no OD register change. |
| 0.3.7 | 2026-08-19 | styles committee ([HYP-1235](https://linear.app/glide-vc/issue/HYP-1235) PR [#736](https://github.com/hyperide/hyper-saas/pull/736)) | **Closes [HYP-1235](https://linear.app/glide-vc/issue/HYP-1235), the follow-up tracked in Rev 0.3.6's closing paragraph — the VS Code extension's write-path pre-check is unified onto the richer A1 detector.** `vscode-extension/hypercanvas-preview/src/services/style-forwarding-check.ts`'s `checkStyleForwarding` (the pre-write ADMIT/EXCLUDE gate `ast-update-utils.ts` calls BEFORE the [Part 7](#part-7--to-be-planner-where-the-value-lives-priority-chain) planner picks a channel — see [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)) previously ran its OWN, older, coarser check (only the target component's own param-destructure shape — no render-body trace, no styled-components recognition, no root-vs-descendant distinction). It now calls `detectForwarding` (`lib/style-read/forward-detect.ts`) directly, via `locateComponentDeclaration` (`lib/style-read/forward-detect-locate.ts`) for the `not-forwarding` definition pinpoint, collapsing the per-channel `ForwardDetectorResult`s into `not-forwarding` only when BOTH channels are a proven (high-confidence) negative — the pre-plan "does ANY channel forward" admit question this call site can answer (the channel THIS write actually uses isn't known yet at this point in the pipeline; that per-channel refusal already happens downstream in the shared executor's HYP-995 `refusalForUnforwardedProps`, `lib/style-write/style-write-executor.ts`). Concretely more accurate: a `styled.button(...)` factory now returns `forwards` (a confident positive, matching the known styled-components library contract) instead of the old check's `unknown`; a component that destructures `className` but attaches it to a nested descendant rather than its returned root (`({ className }) => <div><span className={className}/></div>`) is now correctly excluded (`not-forwarding`) instead of the old destructure-shape-only check wrongly admitting it as `forwards` — both regression-tested in `style-hyp901-hardening.test.ts`, along with a native-tag admit pin and a mixed-confidence collapse case (one channel proven negative, the other genuinely `low` — must stay `unknown`, never `not-forwarding`; a mutant flipping the collapse's `&&` to `||` would otherwise pass every other test in the file). Fixed one real bug found while porting: `locateComponentDeclaration`'s `LocatedComponent` had no reliable declaration-line field — `fnNode?.loc` is `null` for a `FunctionDeclaration` wrapped in `export default function Foo() {}` (a reproduced `recast`/`../ast/parser.ts` quirk: the OUTER `ExportDefaultDeclaration` keeps its `.loc`, the inner node loses it) — added a computed `definitionLine` field (declarator loc, falling back to the enclosing export statement's loc, matching the original coarse check's own fallback) so the AI-fix diagnosis still pinpoints a definition location for a default-exported component (caught live by the existing `AstServiceStyleWriteHyp990.test.ts` C3 integration test, which asserts `diagnosis.componentDefinition.filePath` for exactly this shape). A 3-model `review diff` round (Opus/Fable/k3, Codex seat unavailable — usage limit) on this same PR independently caught a second real gap: the rewired pre-check silently lost the LOCAL monorepo workspace-package resolution fallback the old coarse check had (a `node_modules` symlink whose `package.json` entry is real `.ts(x)` source — the "conloca" case, HYP-995); the shared executor's own HYP-995 check kept its own copy and backstopped it (no live regression), but the finding was real and independently reproduced by all three reviewers. Fixed in the same PR: the fallback moved to a new shared module, `lib/ast/workspace-package-entry.ts`'s `resolveWorkspacePackageEntry`, now called by BOTH `forward-detect-locate.ts`'s `locateComponentDeclaration` and `component-forwarding.ts`'s `resolveComponentForwarding` — a workspace-package component resolves identically through either path, verified by two new `forward-detect.test.ts` cases mirroring `component-forwarding.test.ts`'s existing "conloca case" security tests (traversal-specifier and entry-escape guards untouched, still covered where they already lived). The review also flagged a duplicated `describeJsxName` dotted-name helper (byte-identical copies in `style-forwarding-check.ts` and `component-forwarding.ts`) — extracted to a single shared `jsxNameFull` in `lib/ast/jsx-deps.ts` next to the already-shared `jsxNameRoot`, both call sites updated. **Deliberately NOT done here:** `component-forwarding.ts`'s own PER-PROP forwarding analysis (the destructure-shape read, downstream of location resolution — separate from the location-resolution fallback above, which IS now shared) remains its own separate, older implementation; unifying it onto `detectForwarding`'s richer per-channel render-body trace is still a tracked follow-up, not done here. The 4 in-detector P2s tracked separately in the top-of-doc status line and [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home) ([HYP-1233](https://linear.app/glide-vc/issue/HYP-1233)/[HYP-1234](https://linear.app/glide-vc/issue/HYP-1234) and companions) are unaffected. Verified live: full `bun test` across `lib/`, `vscode-extension/hypercanvas-preview/src/`, `client/`, `server/`, `shared/` (7380+ tests) green; `bun run typecheck` (tsgo) clean; `oxlint`/`knip` clean on touched files; a 3-model `review diff` round (Opus/Fable/k3) reviewed the full staged diff and both real findings (workspace-package fallback loss, duplicated dotted-name helper) were fixed before merge, not deferred. No prescriptive (TO-BE) change; no OD register change. |
| 0.3.8 | 2026-08-19 | styles committee ([HYP-1294](https://linear.app/glide-vc/issue/HYP-1294) PR [#738](https://github.com/hyperide/hyper-saas/pull/738)) | **Closes [HYP-1294](https://linear.app/glide-vc/issue/HYP-1294) — `componentPropSurface` (the A1 forward-detector's facts, fetched into `ElementStyleData` since Rev 0.3.6 via [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280)) had zero live consumers on either platform, flagged as a P1 in HYP-1280's own review. This PR wires it into a real, visible feature.** A new host-agnostic predicate, `hasNoStyleWriteSurface` (`lib/style-write/stylability-ladder.ts`), reports "no known style-write channel at all" without needing the `InspectorSurfaceDecision` the browser mode lacks (HYP-664 gap). A new hook, `useNoStyleWriteSurfaceWarning` (`client/components/RightSidebar/hooks/`), wires it into the Right Sidebar: the moment the selected element's component provably forwards neither `className` nor `style` and has no partial-prop cover, a persistent, dismissible toast fires — "This component may not accept style edits" — deduped per (componentPath, selectedId, tagType), dismissed when the verdict flips or the selection moves on. **Deliberately informational only** — never disables a control, never gates the write path; the D3 stylability ladder ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)) stays STAGED and unconsulted by the live write path, unchanged by this PR. **VS Code parity closed in the same PR:** `componentPropSurface` was SaaS/browser-only since 0.3.6 — `StyleReadService.buildElementFacts` already computed the facts on the VS Code side but never serialized them onto the `styles:response` RPC payload; it now does (`StyleReadService.ts` / `client/lib/platform/types.ts` / `useElementStyleData.ts`), so the same warning fires identically on both platforms. Two real bugs were found and fixed via a two-round, 3-model `review diff` pass (Opus/Fable/GLM; Codex and k3 hit usage limits both rounds): **(1)** a React StrictMode double-invoke bug — the toast's unmount-cleanup effect dismissed the toast but left its dedupe key set, so StrictMode's dev-mode mount→cleanup→remount cycle (which does not destroy the component or its refs) saw the stale key on remount and never recreated the toast, so the warning appeared once then vanished permanently for that element; fixed by clearing both refs together, pinned with a regression test independently verified to fail without the fix (temporarily reverted, re-ran, confirmed red, restored, confirmed green). **(2)** a stale-pairing race — `componentPropSurface`'s reset-on-selection-change in `useElementStyleData.ts` was effect-driven, lagging one render behind `elementId`/`componentPath`, so a consumer reading all three together could see one element's identity paired with the previous element's stale verdict for one render; fixed with a render-time reset (React's documented "adjust state while rendering" pattern) using `useState` for the tracked key rather than `useRef` — a second independent review round (Opus and GLM both) flagged that a ref write survives a discarded/interrupted render pass while a state write does not, so the `useRef` version of the same fix could theoretically desync under a discarded concurrent render. Also from review: the toast's dedupe key now includes `tagType` (a component rename shouldn't leave a toast showing the old tag name); the VS Code RPC merge now keeps the last known-good verdict on an empty re-read response, mirroring the existing `i18nText ?? prev.i18nText` idiom one line above it; a `PanelRouter`-level test now pins that `componentPropSurface` survives the object-spread onto `styles:response` and not just `StyleReadService`'s own return value; and a new `forward-detect.test.ts` case proves an unanalyzable component (e.g. an unresolvable import) projects to a surface that does **not** trigger the warning, closing a high-severity review concern that "could not analyze" might collapse to the same shape as "proven non-forwarding" (it already didn't — `projectForwardDetectionToPropSurface`'s existing low-confidence-never-excludes rule handles this correctly — this test proves the full chain end to end). **Deliberately out of scope:** HYP-1288 (cold-frame priority race, needs real design work), HYP-1292 (className sync gaps), HYP-1293 (ComponentService perf/architecture) — separate, larger, already-tracked follow-ups, not touched here; coordinating this proactive toast with the existing post-write "could not apply" warning (`warningToastRef`/HYP-901/HYP-987) is documented as a known gap in the new hook's own file header, a real follow-up, not silently dropped. Verified live: `bun test --isolate` across every touched directory (677 tests, 0 failures); `bun run typecheck` (tsgo) clean, root and extension; extension `npm run build` (esbuild + webview node-dep check) clean; a real screenshot of the running VS Code extension (built from this branch) showing the toast fire on an actual non-forwarding component in a live preview. No prescriptive (TO-BE) change; no OD register change. |
...
- **B1 runtime-verify — first ext-side SLICE shipped** ([HYP-987](https://linear.app/glide-vc/issue/HYP-987) M1, `c0965448`/#623). A forward-detect +
  ...
  matrix, no SaaS realm). [HYP-991](https://linear.app/glide-vc/issue/HYP-991) (#666, `PostEditDiagnosticWatcher`) extends it and is **merged** (2026-07-17). [HYP-990](https://linear.app/glide-vc/issue/HYP-990)
  M2 (#665, atomic saga + write-scoped verify markers + per-path mutex + native AI-autofix notification,
  grown to also include an **ext-side-only** implementation of the §9.4 confidence×verifiability matrix)
  is **merged** (2026-08-18, `d386cc43`) — the SHARED, both-realm `lib/style-write/runtime-verify/`
  foundation (dual-settle across realms) remains the still-absent thing D19 names above; #665's matrix
  logic lives in the ext-only `ast-update-utils.ts` down-payment, not the shared foundation.
...
recommendations — only the named sub-decisions inside each remain as implementation knobs. (Of the
second-tier forks in [§13.7](#137-od-6-through-od-11--the-second-tier-opens), OD-9 is now CLOSED (merged) — OD-6, OD-7, OD-8, OD-10, OD-11 remain genuinely open.)
...
**Multi-select style write is PARTIAL: SaaS/browser realm shipped on `main`, ext realm still gapped.**
PR [#270](https://github.com/hyperide/hyper-saas/pull/270) merged to `main` 2026-06-18 (commit `4c243f67`; [HYP-271](https://linear.app/glide-vc/issue/HYP-271)/[HYP-581](https://linear.app/glide-vc/issue/HYP-581) Done). `RightSidebar.tsx`
now has `mergeForMultiSelect`, `multiSelectMerged`, `multiSelectData`, batch source-tabs, and MIXED-placeholder
rendering — when the Auto-chip intersection routing succeeds, `selectedIds.length > 1 && multiSelectMerged`
mounts the full multi-select inspector; the historical `selectedId = selectedIds.length === 1 ? selectedIds[0]
: null` gate (still at `RightSidebar.tsx:141`) only governs the pre-merge single-value derivation, and
"N elements selected / Multi-select editing is unavailable for this selection" (`:1498-1499`) is the
fallback rendered when `!multiSelectMerged`, not the universal multi-select behavior. The remaining gaps are
realm and wiring gaps, not a missing feature: the VS Code **extension** realm has no wiring at all (no
`CanvasEngineProvider`/`multiSelectMerged`/`mergeForMultiSelect` in `vscode-extension/hypercanvas-preview/src`
— [HYP-427](https://linear.app/glide-vc/issue/HYP-427), Backlog). Production flush DOES batch-write for real on the SaaS/browser realm today —
`useStyleSync.flushQueue` branches on `selectedIds.length > 1 && engine` and calls
`engine.updateASTStylesBatch(selectedIds, ...)` → `POST /api/update-component-styles-batch`, which resolves
and writes each element freshly server-side — but this live path is NOT yet the frozen, inspectable
`BatchStyleWritePlan` (`batch-style-write-plan.ts` is explicitly staged/not-live-yet per its own file header)
with per-rung channel dispatch + pre-flight `surfaceDecision`; that target lands with [HYP-664](https://linear.app/glide-vc/issue/HYP-664) (Backlog).
Props-mode adapters (Tamagui/RN) are explicitly excluded from the live batch path today (`useStyleSync.ts`
checks `styleAdapter.writeMode === 'props'` and drops the queued write with only a `console.warn`,
never mis-routing raw CSS) — but this exclusion is **silent, not visibly reported**: the "multi-select
editing is unavailable" fallback only renders when `!multiSelectMerged` (defined earlier in this section), a read-side check, and
`TamaguiAdapter.read()` always returns parsed styles unconditionally, so `multiSelectMerged` stays
truthy and the normal multi-select inspector renders with no visible error while writes are dropped —
a live gap ([Part 14.4](#144-acceptance-gate--erroredge-case-matrix) should track a visible exclusion/error path for this case). **Multi-select style write:
PARTIAL (SaaS WORKS-but-unguarded via the live per-element batch route — the route itself functions, but
per [§14.1](#141-sequencing-principle) it is both forwarding-blind and unverified pending A1+B1, the exact failure class D1/D3/D14 name;
ext-parity and the frozen-plan/per-rung-dispatch target PLANNED).**

> **Discrepancies touching this subsystem:** **D7** (multi-select "generalize the one engine" is the
> central directive — Crossrealm-bridge [§3](#part-3-as-is-sections-3137--current-state-topology-adapters-read--write-pipelines)b, the D2/D3 specs, and Alex repeatedly — and on `main` the SaaS/browser
> realm now does generalize into the merged inspector; the batch path is merged, and the residual gap is
> ext-realm parity + frozen-plan dispatch). This is one of the
...
| i18n key read             | Docker: existing-key retarget only (**PARTIAL**); NodePod: **WORKS** ([HYP-372](https://linear.app/glide-vc/issue/HYP-372) Done, PR #477/#478) | `styles:fetchI18nKeys` **WORKS**                                                                                  |

The formerly outright hole — the SaaS i18n-key read — is now substantially closed, but the two SaaS
runtimes do not have equal capability: `styles:fetchI18nKeys` is handled in the extension
(`PanelRouter.ts:760`) and the SaaS server now has an equivalent scan route
(`server/routes/scanI18nBindings.ts`, `server/routes/retargetI18nKey.ts`). In browser mode the scan
result is composed separately into `browserI18nText` (`RightSidebar.tsx:224-233`) — `classData.i18nText`
(the VS Code RPC value read at `useElementStyleData.ts:551`) is still not populated in browser mode.
[HYP-372](https://linear.app/glide-vc/issue/HYP-372) (Done) shipped in two phases — PR #477 (read + existing-key retarget) and PR #478 (new-key
create + full locale-dictionary combobox), but that second phase's full capability is **NodePod-only**:
`RightSidebar.tsx:261` gates `nodePodCanCreate` directly on `nodePodRuntimeMode === 'nodepod'`; `:244`
(`nodePodLocaleKeys`) no-ops outside NodePod internally, inside `useNodePodLocaleKeys` (same effective
result via a different mechanism/location). Docker SaaS gets existing-key retarget only, scanning bindings in the current source file
(`useBrowserI18nText.ts:140-162` scans, `:166-172` derives the retargetable keys) — it cannot create missing keys or browse the full locale dictionary.
**PARTIAL for Docker SaaS, WORKS for NodePod**; residual hardening (NodePod OpfsFileStore durability,
SW-intercept edge cases) is tracked separately in [HYP-746](https://linear.app/glide-vc/issue/HYP-746) (Backlog).

> **Discrepancies touching this subsystem:** **D10** — RESOLVED: `styles:fetchI18nKeys` exists in VS Code
> and the SaaS server now has an equivalent route ([HYP-372](https://linear.app/glide-vc/issue/HYP-372) Done, PR #477/#478); the i18n combobox is no
> longer hidden in browser mode. Style-adjacent but in
...
| VS Code ElementFacts (sourceOwners/propMappers/themeCaps/computedStyle)        | **PARTIAL**                                    | `sourceOwners`/`propMappers`/`themeCaps` still hardcoded empty; className/style forwarding is NO LONGER hardcoded — A1's `detectForwarding` is MERGED and wired at `buildElementFacts` ([HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719), `860f66c1`, 2026-08-18) on the VS Code side; the SaaS/browser side now also calls it via the new `GET /api/element-forwarding` route ([HYP-1280](https://linear.app/glide-vc/issue/HYP-1280)); the detector's own 4 P2 refinements (declared-prop-as-proof, multi-return-path, Slot branches, transformed-binding exclusions) remain open follow-ups, see [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Multi-select style write                                                       | **PARTIAL**                                    | SaaS shipped on `main` ([#270](https://github.com/hyperide/hyper-saas/pull/270)/[HYP-271](https://linear.app/glide-vc/issue/HYP-271), merged 2026-06-18); ext realm unwired ([HYP-427](https://linear.app/glide-vc/issue/HYP-427) Backlog); frozen-plan dispatch incomplete ([HYP-664](https://linear.app/glide-vc/issue/HYP-664) Backlog)                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| B1 runtime-verify (did the write land) + fail-closed matrix                    | **PARTIAL (M1 ext-slice)**                     | Ext-side verify-and-retry shipped ([HYP-987](https://linear.app/glide-vc/issue/HYP-987) M1, #623) in `services/ast-update-utils.ts` — forward-detect + auto-wrap + before/after computed-style diff that warns+rolls back a non-forwarding write; a down-payment, NOT the shared foundation. `lib/style-write/runtime-verify/` (the SHARED, both-realm dual-settle foundation) still absent (D19). [HYP-991](https://linear.app/glide-vc/issue/HYP-991) (#666, PostEditDiagnosticWatcher) **merged** 2026-07-17. [HYP-990](https://linear.app/glide-vc/issue/HYP-990) M2 (#665, atomic saga + verify markers + AI-autofix notify + an **ext-side-only** implementation of the §9.4 confidence×verifiability matrix) **merged** 2026-08-18 (`d386cc43`) — still no SaaS realm, still no shared/cross-realm matrix. |
| Cross-realm StateHub / PanelRouter / AstBridge transport                       | **WORKS**                                      | [§3.13](#313-cross-realm-transport-today)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| SaaS i18n key read                                                             | **PARTIAL for Docker SaaS, WORKS for NodePod** | [HYP-372](https://linear.app/glide-vc/issue/HYP-372) Done (PR #477/#478) — Docker SaaS gets existing-key retarget only (current-file scan, no create, no locale dictionary); NodePod gets full parity; see [§3.13](#313-cross-realm-transport-today). Residual hardening tracked in [HYP-746](https://linear.app/glide-vc/issue/HYP-746) Backlog                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
adapter limitation (**will-not-fix**, subsumed by D23); and **D10** (the SaaS i18n-key-read route) is
now **RESOLVED** — [HYP-372](https://linear.app/glide-vc/issue/HYP-372) shipped the transport row (PR #477/#478). Everything else in D1-D11 is fully resolved by the pipeline.
...
| **D7**  | Multi-select "generalize the one engine" is the central directive; on `main` the SaaS/browser realm now DOES generalize (the live batch route — `useStyleSync.flushQueue` → `engine.updateASTStylesBatch` → `POST /api/update-component-styles-batch` — is v1-MERGED to `main` via [#270](https://github.com/hyperide/hyper-saas/pull/270); `BatchStyleWritePlan`/`recordBatchEdit`/L0-L3 scaffolding is merged too, but the `ast:updateStylesBatch` ext-host message it targets is STAGED, not live per `batch-style-write-plan.ts`'s own file header), but the ext realm remains single-element-only ([HYP-427](https://linear.app/glide-vc/issue/HYP-427) Backlog) and production flush is not fully on the frozen-plan path ([HYP-664](https://linear.app/glide-vc/issue/HYP-664) Backlog). | **resolved-by-Part-11 (one engine, vectorized; single-select = `length===1`)**. Multi-select is the N≥1 generalization through `mergeSubjects()`/per-element resolution, never a parallel batch system; the batch plan is a frozen aggregate of independent single-element decisions. One of the four un-dodgeable reconciliations. Source: Q6 Agreement [§1](#part-1--executive-summary); [Part 11](#part-11--to-be-multi-select-model--stylability-ladder--wrapper-promotion).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **D10** | `styles:fetchI18nKeys` existed in VS Code (`PanelRouter.ts:760`) but the SaaS server handler was MISSING; `classData.i18nText` was never populated in SaaS, the i18n combobox was hidden in browser mode.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **RESOLVED-WITH-A-KNOWN-FORK — [HYP-372](https://linear.app/glide-vc/issue/HYP-372) shipped (Done, PR #477/#478)**. i18n-key read is no longer BROKEN/MISSING in either realm; `StyleReadService._tryDetectI18n` was ported to a SaaS route (`server/routes/scanI18nBindings.ts`/`retargetI18nKey.ts`). The two realms now differ in CAPABILITY, not only transport: Docker SaaS composes a separate `browserI18nText` value via current-file scanning (existing-key retarget only, no create, no locale dictionary), while NodePod gets full parity (client-only OPFS) — **PARTIAL for Docker SaaS, WORKS for NodePod**, see [§3.13](#313-cross-realm-transport-today). Residual hardening (durability, SW-intercept edge cases) tracked in [HYP-746](https://linear.app/glide-vc/issue/HYP-746) (Backlog). Source: AS-IS [§2](#part-2--glossary--term-decode)c/[§7](#part-7--to-be-planner-where-the-value-lives-priority-chain); [Part 5.4](#54-realm-model--three-first-class-realms-as-transport-rows-over-one-contract). |
...
| **D29** | PropsEditor + Tamagui tokens must be SHARED ext↔SaaS; [HYP-709](https://linear.app/glide-vc/issue/HYP-709)/716 work is DONE and the PR ([#453](https://github.com/hyperide/hyper-saas/pull/453), ex-[#435](https://github.com/hyperide/hyper-saas/pull/435)) is **merged to `main`** ([#593](https://github.com/hyperide/hyper-saas/pull/593) re-landed the orphaned #453 squash commit on 2026-07-01, commit `0e27bc12`; PR #453's own commit `23ed146c` from 2026-06-12 never reached `origin/main` after a history rewrite). Alex ([§8](#part-8--to-be-fallback-doctrine-vtswr)): "в ext должен использоваться этот код тоже." On `main` the ext webview now shares the PropsEditor (`RightSidebar.tsx`'s `PropsSection`/`PropsEditor` renders via the platform-converged `usePropsEditorSource` seam, no `useCanvasEngine()` dependency). | **resolved — OD-9 closed**. PR [#453](https://github.com/hyperide/hyper-saas/pull/453) (ex-[#435](https://github.com/hyperide/hyper-saas/pull/435)) merged, landing [HYP-709](https://linear.app/glide-vc/issue/HYP-709)/[HYP-716](https://linear.app/glide-vc/issue/HYP-716); behavior specced in Part-12.2 is now the shipped behavior. The work is complete (shared `lib/tamagui/extract-tokens.ts`, `TokenCombobox`, 5 green Docker e2e) and merged; no further action pending. Source: D29; [Part 12.2](#122-token-providers--the-project-palette-gap), [Part 13](#part-13--decision-register-od-1od-5-ratified-by-cto-od-6od-11-open) OD-9.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **D34** | Multi-select style write: unit tests DO exist for the shipped SaaS batch route (`server/routes/updateComponentStylesBatch.test.ts` 16 tests; `useStyleSync.test.tsx` 3 multi-select batch cases + 1 single-select-does-NOT-batch guard) — this was stale per D7's own correction elsewhere in this doc. The real remaining gap is narrower: no e2e coverage, and no test for the still-STAGED `ast:updateStylesBatch` ext-host message/frozen-plan path (unbuilt per D7/§9.2a).                     | **Part-14.4** — e2e batch coverage + tests for the frozen-plan/ext-host path land WITH the ext-realm generalization ([Part 11](#part-11--to-be-multi-select-model--stylability-ladder--wrapper-promotion), gated on A1 per [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)). Gates D7. Source: D34; [Part 11](#part-11--to-be-multi-select-model--stylability-ladder--wrapper-promotion), [Part 14.4](#144-acceptance-gate--erroredge-case-matrix).                   |
...
1. **ONE engine; multi-select is the N≥1 generalization, never a parallel batch system — and it is
   ...
   undoable) is preserved unweakened. On main the multi-select N≥1 generalization is **PARTIAL**
   (SaaS/browser realm shipped via [#270](https://github.com/hyperide/hyper-saas/pull/270); ext realm
   remains single-element-only, [HYP-427](https://linear.app/glide-vc/issue/HYP-427) Backlog — see [§3.9](#39-modes--jsx-vs-dom-single-vs-multi)/D7), and this invariant is the
   rule that keeps the shipped slice from regrowing as a parallel system at the UI layer.
...
| i18n-key read                                                                                                                   | **shipped ([HYP-372](https://linear.app/glide-vc/issue/HYP-372) Done, PR #477/#478)**: `scanI18nBindings.ts`/`retargetI18nKey.ts` route, current-file scan only (existing-key retarget, no create, no locale dictionary) — separate `browserI18nText` composition, `classData.i18nText` stays unpopulated | `styles:fetchI18nKeys` RPC (`PanelRouter.ts:760`)                                  | in-pod read of the OPFS project (same `_tryDetectI18n` logic, no server hop) — full parity: create + locale dictionary | **Yes — Docker SaaS is a distinct, narrower capability, not just a different transport (D10)**                                                                                                                                       |
...
the ext host, `StyleReadService.ts:213`; the wider "garbage facts" — `buildElementFacts:763`,
`getCssSystems:774` — AS-IS [§2](#part-2--glossary--term-decode)c) is therefore an artifact of the
...
degrades in the TO-BE target is **serverless SaaS (NodePod/OPFS)** on the LSP/type-backstop row alone —
today's AS-IS table above also flags a SECOND, narrower AS-IS-only degrade (Docker SaaS i18n
key-create/locale-dictionary, [§14.4](#144-acceptance-gate--erroredge-case-matrix), untickered), which is a capability gap to CLOSE, not a sanctioned target-state
degrade; there the rule is
...
projection is not re-derived per realm or per design system. All three **already hold in code today**
(props editing is universal and positioned below the styles form, and — since PR [#453](https://github.com/hyperide/hyper-saas/pull/453)/OD-9 reached `main`
via #593 on 2026-07-01 — this now holds on the ext realm too, confirmed by `RightSidebar.tsx`'s `PropsSection`
rendering `<PropsEditor/>` via the platform-converged `usePropsEditorSource` hook rather than the
throwing `useCanvasEngine()`):
...
1. **Props editing is UNIVERSAL, not Tamagui-only — it does NOT gate on the design system.** The props
   ...
   element), and **(b)** a **TypeScript prop schema** being readable for that component — NOT on
   an engine being present (`usePropsEditorSource`'s `useCanvasEngineOptional()` falls back to the
   ext realm's `SharedEditorState` path when no `CanvasEngineProvider` exists) and NOT on which
   `designSystem` the project uses. Tamagui only
...
3. **The former ext-realm gap is CLOSED.** PR [#453](https://github.com/hyperide/hyper-saas/pull/453) (ex-[#435](https://github.com/hyperide/hyper-saas/pull/435)) reached `main` via [#593](https://github.com/hyperide/hyper-saas/pull/593) on 2026-07-01 (commit `0e27bc12`; PR #453's own 2026-06-12 commit `23ed146c` never landed on `origin/main` — a history rewrite orphaned it, and #593 re-landed the squash), landing the
   shared `PropsEditor` + `TokenCombobox` + Tamagui-token machinery ([HYP-709](https://linear.app/glide-vc/issue/HYP-709)/[HYP-716](https://linear.app/glide-vc/issue/HYP-716), Done). The VS Code
   **extension** webview now renders `<PropsEditor/>` (via `RightSidebar.tsx`'s `PropsSection`), sourcing
   selection/AST/schema/tokens through the platform-converged `usePropsEditorSource` seam instead of
   calling the throwing `useCanvasEngine()` internally (as `PropsEditor` previously did). Because that
   internal call used to throw in the ext webview (no `CanvasEngineProvider` there), `RightSidebar.tsx`
   previously wrapped the `<PropsEditor/>` call site in an `engine &&` guard (reading the OUTER, safe
   `useCanvasEngineOptional()` result) purely to keep `PropsEditor` from ever mounting — and throwing —
   on the ext realm; that guard is now gone, since `PropsEditor` no longer throws there. `PropsEditor`
   self-gates on having a file path + a typed props schema instead. This is exactly D29 / **OD-9** ([§4.4](#44-intentspec-tensions-d24-d29), [§12.2](#122-token-providers--the-project-palette-gap), [§13.7](#137-od-6-through-od-11--the-second-tier-opens)): the disposition was
   a **merge**, not a design fork, and the merge has landed. The invariants above are the SaaS behavior
   today and now hold on the ext realm as well.
...
[§3](#part-3-as-is-sections-3137--current-state-topology-adapters-read--write-pipelines)b/[§3](#part-3-as-is-sections-3137--current-state-topology-adapters-read--write-pipelines)c above have a working single-file undo; NEITHER realm's _editor undo/redo stack_ is wired to a
...
`StyleReadService.ts:213`; the wider "garbage facts" limitation is `buildElementFacts:763` /
`getCssSystems:774`, AS-IS [§2](#part-2--glossary--term-decode)c)? The answer is **round-trip
...
home is [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)"), and the [Part 14.2](#142-phase-map-with-the-live-tickets) Phase-3 row all cite [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home); no other section re-specifies the detector. (**On `main` A1 is now BUILT and MERGED** —
`StyleReadService.buildElementFacts` no longer hardcodes `acceptsClassName:true`; it calls the real
detector, [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719), `860f66c1`, 2026-08-18; Rev 0.3.6.)

[HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719)
(`lib/style-read/forward-detect.ts` + `forward-detect-type.ts` + `forward-detect-trace.ts`, matching the
target module below) **MERGED to `main` 2026-08-18** (`860f66c1`). Its Codex review had left an
unaddressed **P1** at merge time — the detector was invoked only from the VS Code RPC service
(`StyleReadService.ts`); the SaaS/browser read path (`client/lib/platform/hooks/useElementStyleData.ts`)
read canvas-engine/DOM facts and never called it, so the SaaS realm was exactly as blind as before the
merge. That P1 is now CLOSED — [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280) added a
`GET /api/element-forwarding` server route (`server/routes/readElementForwarding.ts`) the browser-mode
`useElementStyleData.ts` fetches asynchronously, wiring the SaaS realm onto the same `detectForwarding`
call and the same `projectForwardDetectionToPropSurface` projection VS Code uses (moved into
`lib/style-read/forward-detect.ts` so both platforms share one projection). 4 P2s from the same review
remain open, tracked as their own follow-ups: declared-but-unforwarded prop types treated as forwarding
proof; a transformed-yet-still-referenced destructured binding wrongly emitted as a high-confidence
negative exclusion; only the last JSX-like `return` is inspected, missing earlier conditional-return
paths; and any `Slot`-importing component is assumed to forward both channels without verifying either
branch does. **A1 merging (and HYP-1280 closing) does NOT by itself unblock Phase 3** — the
[Part 14.2](#142-phase-map-with-the-live-tickets) table's `A1 → B1` hard dependency still applies, and the full B1 foundation
(dual-settle, fail-closed matrix, both realms) remains PLANNED per [§3.15](#315-as-is-subsystem-status-roll-up), independent of A1's own status.

**Write-path consumption ([HYP-1235](https://linear.app/glide-vc/issue/HYP-1235), CLOSED Rev 0.3.7):**
this section's opening paragraph names "the pre-write capability check the planner (A2, [Part 7](#part-7--to-be-planner-where-the-value-lives-priority-chain)) consumes" — on the VS Code extension, that
consumer is `vscode-extension/hypercanvas-preview/src/services/style-forwarding-check.ts`'s
`checkStyleForwarding`, called BEFORE the planner picks a channel. It previously ran its OWN, older,
coarser check (only the target component's own param-destructure shape — no render-body trace, no
styled-components recognition, no root-vs-descendant distinction) rather than this section's detector. It
is now wired directly onto `detectForwarding`, collapsing the per-channel `ForwardDetectorResult`s into
`not-forwarding` only when BOTH channels are a proven (high-confidence) negative — the pre-plan "does ANY
channel forward" admit question this call site can answer (the channel a write actually uses isn't known
until the planner runs). A 3-model `review diff` round on the closing PR independently caught (Opus/
Fable/k3, unprompted) that this rewiring silently dropped a real capability the old coarse check had: the
LOCAL monorepo workspace-package fallback (a `node_modules` symlink whose `package.json` entry is real
`.ts(x)` source — the "conloca" case, HYP-995). Fixed in the same PR by extracting it to a new shared
module, `lib/ast/workspace-package-entry.ts`'s `resolveWorkspacePackageEntry` — both this detector's own
resolver (`forward-detect-locate.ts`'s `locateComponentDeclaration`) and the shared executor's
channel-precise HYP-995 write refusal (`lib/style-write/component-forwarding.ts`, consumed by BOTH
platforms via `lib/style-write/style-write-executor.ts`) now call the SAME fallback, so a workspace-package
component resolves identically through either path — no more capability gap between the two, and one
fewer traversal-guarded resolution copy to keep in sync. `component-forwarding.ts`'s own PER-PROP analysis
(the destructure-shape read, downstream of location resolution) remains its own separate, older
implementation — unifying THAT onto `detectForwarding`'s richer per-channel trace is still a tracked
follow-up, not done here.

**Proactive read-path consumption ([HYP-1294](https://linear.app/glide-vc/issue/HYP-1294), CLOSED —
follow-up to [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280)):** HYP-1280 (Rev 0.3.6) wired
`componentPropSurface` (this section's detector output, projected via
`projectForwardDetectionToPropSurface`) onto `ElementStyleData` in BOTH the SaaS browser read path and
VS Code, but that PR's own review flagged it as inert — nothing shipped actually READ the field. This
follow-up is the field's FIRST live consumer: `client/components/RightSidebar/hooks/
useNoStyleWriteSurfaceWarning.tsx` raises a persistent, dismissible toast the moment
`lib/style-write/stylability-ladder.ts`'s `hasNoStyleWriteSurface` reports NO known style-write channel
at all for the selected element (neither a generic className/style/css/sx channel nor a partial
styleLikeProps/semanticProps cover) — BEFORE any write is attempted, not as a reaction to one failing.
This is deliberately **informational only**: it never disables a style control and does not gate the
live write path — that write-time enforcement is the separate, still-STAGED D3 stylability ladder
([§4.1](#41-speccode-discrepancies-d1-d11) D3; `resolveStyleSurface` is not consulted by the live
`ast:updateStylesBatch` handler yet). VS Code mode previously left `componentPropSurface` unset on
`ElementStyleData` (the ext computed the facts internally but never serialized them onto the
`styles:response` RPC payload) — this PR closes that gap too: `StyleReadService.
ElementStyleReadResult` now carries `componentPropSurface` (already computed by `buildElementFacts`,
no new detection work), so the same proactive warning fires identically on both platforms. Source:
[PR #738](https://github.com/hyperide/hyper-saas/pull/738); regression tests in
`lib/style-write/stylability-ladder.test.ts`, `client/components/RightSidebar/hooks/__tests__/
useNoStyleWriteSurfaceWarning.test.tsx`, `client/lib/platform/hooks/useElementStyleData.forwarding.test.ts`,
`vscode-extension/hypercanvas-preview/src/__tests__/StyleReadService.test.ts` and `PanelRouter.test.ts`,
and `lib/style-read/forward-detect.test.ts` (pins that an UNANALYZABLE component, e.g. an unresolvable
import, fails open and does not trigger the warning — a two-model review round flagged this as the
one place a wrong projection would be user-visible for the first time).
...
1. **Typed inspector** (the biggest visible win — "it knows my components"). Real prop types, unions,
   ...
   endpoint. What is still greenfield is the _serverless_ transport of the same capability
   (serverless has no LS yet — the ext realm's props editor already shares the SaaS `PropsEditor` UI as
   of PR [#453](https://github.com/hyperide/hyper-saas/pull/453)/OD-9, reached `main` via #593 on 2026-07-01) and the unification under
   the facade.
...
**This is already partly what is on main (D7), with the ext realm the residual gap.** The multi-select
selection/merge UI — `mergeForMultiSelect`, `multiSelectMerged`, `multiSelectData` — is v1-merged to
`main` ([#270](https://github.com/hyperide/hyper-saas/pull/270), 2026-06-18, commit `4c243f67`; [HYP-271](https://linear.app/glide-vc/issue/HYP-271)/[HYP-581](https://linear.app/glide-vc/issue/HYP-581) Done) and wired live in `RightSidebar.tsx`
for the SaaS/browser realm. The frozen `BatchStyleWritePlan` / `recordBatchEdit` / `ast:updateStylesBatch`
host-handler / L0–L3 ladder merged in the same PR is **staged, not yet on the live write path**
(`batch-style-write-plan.ts:4-10`) — the live flush today is `useStyleSync.flushQueue` →
`engine.updateASTStylesBatch` → `POST /api/update-component-styles-batch`, which resolves and writes
each element freshly server-side rather than building/freezing a plan (see below); the historical
single-element gate (`selectedId = selectedIds.length === 1 ? selectedIds[0] : null`, `RightSidebar.tsx:141`)
now only derives the pre-merge single value, and the "N elements selected / Multi-select editing is
unavailable for this selection" fallback text (`:1498-1499`) shows only when `!multiSelectMerged`. **Status: PARTIAL.** What remains PLANNED is (a) the
VS Code **extension** realm, which has zero wiring of this ladder — its dependency edge was "starved
until the A1 forward-detector lands" ([Part 14.2](#142-phase-map-with-the-live-tickets); canonical home is [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)); A1 has now
LANDED ([HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) PR [#719](https://github.com/hyperide/hyper-saas/pull/719), `860f66c1`, 2026-08-18, Rev 0.3.6) so that specific edge is
cleared, but [HYP-427](https://linear.app/glide-vc/issue/HYP-427) itself (the ext-realm ladder wiring) is still unbuilt (Backlog), and (b) production
flush being on the frozen, inspectable `BatchStyleWritePlan` + per-rung dispatch + pre-flight
`surfaceDecision` path — today's live multi-select flush (`useStyleSync.flushQueue` →
`engine.updateASTStylesBatch` → `POST /api/update-component-styles-batch`) resolves and writes each
element freshly server-side rather than freezing/inspecting a plan first ([HYP-664](https://linear.app/glide-vc/issue/HYP-664), Backlog). The TO-BE here is the target the
[#270](https://github.com/hyperide/hyper-saas/pull/270) branch was reaching for, corrected for the as-built defects the discovery surfaced, and generalized to the
ext realm.
...
> shows Radix, D29 shared-PropsEditor width/datalist nits — PR reached `main` via #593 on 2026-07-01). The unbuilt Tier-2 ([§12.4](#124-tier-2-where-in-source--the-per-css-approach-candidate-strategies)) is NOT solved
...
**The shared-code gap (D29). RESOLVED — work done, PR merged.** The intent ([§8](#part-8--to-be-fallback-doctrine-vtswr),
...
5 green Docker e2e) and the PR ([#453](https://github.com/hyperide/hyper-saas/pull/453), ex-[#435](https://github.com/hyperide/hyper-saas/pull/435)) reached `main` via [#593](https://github.com/hyperide/hyper-saas/pull/593) on 2026-07-01 (commit `0e27bc12`; #453's own 2026-06-12 commit `23ed146c` was orphaned by a history rewrite and never reached `origin/main`), so on `main` the ext
now shares the PropsEditor. The TO-BE position was that this is not a new design, it is a merge —
the master spec recorded it as a ratification/landing item (OD-9, [Part 13.7](#137-od-6-through-od-11--the-second-tier-opens)), and the merge has
landed. Two UI nits attached on the branch (Tamagui datalist of 321 options called
"лишнее"; dropdown not width-matched to the select) were addressed there.
...
**The width/datalist nits (D29, attached).** On the shared-PropsEditor work the Tamagui
token datalist (321 options) was flagged as "лишнее" and the dropdown was not width-matched to
the select; both are addressed on [#453](https://github.com/hyperide/hyper-saas/pull/453) (reached `main` via #593 on 2026-07-01) — folded into the same landing item, not a
separate design.
...
> the now-settled frame. Of the second-tier forks in the OD-6..OD-11 numbered range ([§13.7](#137-od-6-through-od-11--the-second-tier-opens)),
> **OD-9 is now CLOSED** (the shared-PropsEditor merge, revision-table row 0.3.3) — the remaining
> **OD-6, OD-7, OD-8, OD-10, OD-11 remain genuinely OPEN**, each still only a recommendation awaiting
> sign-off. Each OD-1..5 row is written so the reader sees
...
second-tier forks that get one row each in [§13.7](#137-od-6-through-od-11--the-second-tier-opens) — **OD-9 is CLOSED** (revision-table row 0.3.3), **OD-6, OD-7,
OD-8, OD-10, OD-11 remain OPEN**. Note the [§1.3](#13-the-five-headline-decisions--ratified) NARRATIVE order is not
...
none gates a whole build phase the way the headline five do. **OD-9 is no longer one of the open
forks** — it closed by execution (PR [#453](https://github.com/hyperide/hyper-saas/pull/453) merged, see below); the genuinely open set in this section is
OD-6, OD-7, OD-8, OD-10, OD-11 (five, not six).
...
**OD-9 — Merge the shared-PropsEditor PR (D29). CLOSED — merged.** _Question (as originally posed):_ do
we merge **PR [#453](https://github.com/hyperide/hyper-saas/pull/453) (ex-[#435](https://github.com/hyperide/hyper-saas/pull/435)), which
...
([§8](#part-8--to-be-fallback-doctrine-vtswr)): "в ext должен использоваться этот код тоже … и PropsEditor.tsx и токены." The work was DONE
([HYP-709](https://linear.app/glide-vc/issue/HYP-709)/[HYP-716](https://linear.app/glide-vc/issue/HYP-716) — shared `lib/tamagui/extract-tokens.ts`, `TokenCombobox`, 5 green Docker e2e), and
PR [#453](https://github.com/hyperide/hyper-saas/pull/453) (ex-[#435](https://github.com/hyperide/hyper-saas/pull/435)) **reached `main` via [#593](https://github.com/hyperide/hyper-saas/pull/593) on 2026-07-01** (commit `0e27bc12`; #453's own 2026-06-12 commit `23ed146c` was orphaned by a history rewrite and never landed on `origin/main`), so on `main` the ext now shares the PropsEditor. _Outcome:_ the
recommendation to merge was followed — this was not an architectural fork, it was a finished branch that
landed, and it directly serves the document's "one engine, three realms" invariant (the inspector
projection is shared, not realm-forked). The Tamagui datalist (321 options) Alex called "лишнее" and the
dropdown width-match were addressed on the branch before merge. _Result:_ the **ext webview now has the
PropsEditor UI** — `RightSidebar.tsx`'s `PropsSection` renders `<PropsEditor/>` via the platform-converged
`usePropsEditorSource` seam (no `CanvasEngineProvider` dependency), closing the System-A/System-B
duplication gap OD-3 targets. _Unblocked:_ shared inspector projection ([Part 5.3](#53-the-convergence-target--system-a-and-system-b-become-one), [Part 6.5](#65-surface-decision--per-property-editability), [Part 12.2](#122-token-providers--the-project-palette-gap)) now
holds on the ext realm — all three props-editor invariants in [§6.5](#65-surface-decision--per-property-editability) (props editing is universal, not
Tamagui-gated; props always below the styles form; the former ext gap is closed) hold for both realms.
...
- **A1 (forward-detector)** gates the remaining multi-select ladder work. The D2/D3 batch core is
  already merged to `main` ([#270](https://github.com/hyperide/hyper-saas/pull/270), [HYP-271](https://linear.app/glide-vc/issue/HYP-271), 2026-06-18) and live for the SaaS/browser realm; the
  starvation on garbage forward-detection facts (`acceptsClassName:true` / `elementPropMappers:[]`
  hardcoded, D3, `StyleReadService.buildElementFacts:763`) is specifically an **ext-host `ElementFacts`**
  problem — `StyleReadService.buildElementFacts` and its hardcoded fields exist only in the ext-host code
  path; the SaaS/browser engine has no equivalent and instead reads live DOM facts directly (`TailwindAdapter.read`
  preferring the live DOM className over the AST, `mergeRuntimeStyle` patching in `getComputedStyle` values —
  [§3.4](#34-read-pipeline--client-hub)), so it is not blind to CURRENT-STATE facts in the same way. A1
  replaces the ext-host's hardcoded facts with real per-channel forwarding evidence — until it lands,
  generalizing the ext realm's ladder to N>1 (once [HYP-427](https://linear.app/glide-vc/issue/HYP-427) wires it) would only generalize that realm's
  blindness. **This is a narrower claim than A1's own forwarding-detection scope, not an exemption**:
  reading live DOM/computed facts tells the SaaS engine what styles currently apply, not whether a
  component forwards the channel a write would target. [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home) is explicit that even after #719 merges,
  "a merge as-is would leave the SaaS realm exactly as blind as before" on forwarding-detection, since
  #719 wires only into the ext-host RPC service and the SaaS read path never calls it — SaaS also has no
  B1 verify at all (PARTIAL, ext-only per the row above), so a SaaS batch write today is both
  forwarding-blind and unverified. The ext-only ElementFacts problem above is a DIFFERENT, narrower gap
  than SaaS's forwarding-blindness — fixing one does not fix the other.
...
> `lib/style-write/runtime-verify/` foundation** (still absent per D19; no dual-settle, no fail-closed
> matrix, no SaaS realm). [HYP-991](https://linear.app/glide-vc/issue/HYP-991) (#666) extends it and has **merged** (2026-07-17). [HYP-990](https://linear.app/glide-vc/issue/HYP-990)
> M2 (#665) extends it further — including now an **ext-side-only** implementation of the §9.4
> confidence×verifiability matrix — and has **merged** (2026-08-18, `d386cc43`). The full
> Phase-2 B1 (SHARED, both-realm dual-settle + fail-closed confidence×verifiability matrix) remains PLANNED;
> #665's matrix is ext-only, inside the down-payment named above, not this shared foundation.
> The [§3.15](#315-as-is-subsystem-status-roll-up) roll-up rows are updated to match.
>
> **A1 forward-detector (Phase 3):** [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229)
> PR [#719](https://github.com/hyperide/hyper-saas/pull/719) (`lib/style-read/forward-detect*.ts`)
> **MERGED to `main` 2026-08-18** (`860f66c1`) — **On `main`, A1 is now BUILT**
> ([§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)). Its Codex review's 1 P1 (wired into
> the VS Code RPC service only; the SaaS/browser read path in
> `client/lib/platform/hooks/useElementStyleData.ts` never called it) is CLOSED —
> [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280) wired the SaaS realm onto the same detector via
> a new `GET /api/element-forwarding` route. 4 P2s remain open (declared-prop-as-proof, unhandled
> multi-return-path, unverified Slot branches, a transformed-binding false exclusion — full list in
> revision row 0.3.4). **Merging #719 (done) and closing HYP-1280 (done) is necessary but not
> sufficient** to unblock Phase 3: the table's `A1 → B1` hard dependency still applies, and the full B1
> foundation is itself still PLANNED (Phase-2 row above).

| Phase                                                                      | Capability (TO-BE)                                                                                                                                                                                                                                                                                                                                                                                | Building ticket(s)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Hard dependency                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Spec part               |
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| **0 — Hygiene & taxonomy**                                                 | Kill stale-fact claims (D19); rename `uiKit→designSystem`, split `projectUIKit` into orthogonal axes (D26); draft the (as-of-writing untickered) B0/B1/B2/B3 foundation tickets (A1 tickered as [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) since Rev 0.3.4, this item is done for A1)                                                                                                 | [HYP-299](https://linear.app/glide-vc/issue/HYP-299) (umbrella), D26 rename (NEEDS-LINEAR), foundation-ticket creation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | none — pure prep                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 4.3, 5.5, 13.6          |
...
| **2 — Verify + fallback doctrine**                                         | B1 verify-everywhere (dual settle, read-frame guards, ext via preview-iframe RPC); VTSWR fallback; fail-closed confidence×verifiability matrix; flip findRule-miss hard-fail → gated inline floor ([HYP-706](https://linear.app/glide-vc/issue/HYP-706) pulled FORWARD here — see the note below)                                                                                                 | B1 foundation still PLANNED (dep B0, now shipped); a first ext-side verify-and-retry SLICE shipped ([HYP-987](https://linear.app/glide-vc/issue/HYP-987) M1, #623), extended by [HYP-991](https://linear.app/glide-vc/issue/HYP-991) (#666, **merged** 2026-07-17) and [HYP-990](https://linear.app/glide-vc/issue/HYP-990) M2 (#665, **merged** 2026-08-18, `d386cc43`) — NOT the shared `lib/style-write/runtime-verify/` foundation, [HYP-706](https://linear.app/glide-vc/issue/HYP-706) (findRule-miss → inline floor) — [HYP-553](https://linear.app/glide-vc/issue/HYP-553) (per-class confidence) already merged 2026-06-17 (PR #468), shipped ahead of its formal phase slot | B1 → B0; VTSWR → B1; [HYP-706](https://linear.app/glide-vc/issue/HYP-706) → B1 only (the floor is _gated_ on B1-verify per [HYP-544](https://linear.app/glide-vc/issue/HYP-544) §A3; it does NOT depend on A1)                                                                                                                                                                                                                                                                                                                                               | 8, 9.2-9.4              |
| **3 — AI ladder + Tier-2 source resolution + color UI**                    | A1 forward-detector (real per-channel forwarding facts, [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)); Tiers 0-5 AI/deterministic ladder (AI routes/ranks, probe is the gate); host-side Tier-2 CSS write-target resolution; static cva-variant resolver; **the project-independent ColorCombobox composition test that picks a color and asserts the source edit landed (D30)** | [HYP-1229](https://linear.app/glide-vc/issue/HYP-1229) (PR [#719](https://github.com/hyperide/hyper-saas/pull/719) **MERGED** 2026-08-18 `860f66c1`; SaaS read-path P1 closed via [HYP-1280](https://linear.app/glide-vc/issue/HYP-1280); 4 P2 refinements remain open; dep B1 (still PLANNED — see Phase-2 row), [§9.2a](#92a-a1--the-forward-detector-its-one-canonical-home)), [HYP-704](https://linear.app/glide-vc/issue/HYP-704) (CSS/var/module exact-only + floor), [HYP-705](https://linear.app/glide-vc/issue/HYP-705) (cva resolver), [HYP-686](https://linear.app/glide-vc/issue/HYP-686) (intent→token, later), [HYP-349](https://linear.app/glide-vc/issue/HYP-349)-followup (color composition test, D30)                                                                                                 | A1 → B1; [HYP-704](https://linear.app/glide-vc/issue/HYP-704)/705 → A1 + B1 (DO NOT start 704/705 before B0/B1 exist — discovery-tickets §[HYP-544](https://linear.app/glide-vc/issue/HYP-544)); D30 test → B1 (asserts landing)                                                                                                                                                                                                                                                                                                                             | 7, 9.2a, 10, 12.4, 12.5 |
| **4 — Multi-select generalization + journal undo**                         | `StyleWriteEngine.apply(selection[], patch)` with single = `length===1`; frozen BatchPlan; write-ahead journal undo (one atomic step across files); per-element resolved-channel observability                                                                                                                                                                                                    | [HYP-271](https://linear.app/glide-vc/issue/HYP-271) **Done** (SaaS/browser realm shipped independent of A1, PR [#270](https://github.com/hyperide/hyper-saas/pull/270)/`main`), [HYP-596](https://linear.app/glide-vc/issue/HYP-596) (proper N>1 rebuild), [HYP-664](https://linear.app/glide-vc/issue/HYP-664) (wire frozen BatchPlan into the flush), [HYP-427](https://linear.app/glide-vc/issue/HYP-427)/422 (ext parity + post-batch refresh), [HYP-661](https://linear.app/glide-vc/issue/HYP-661)/663 (audit + counters)                                                                                                                                                      | all → A1 (un-starve the ladder) + B0 (the journal) — **status note, not a re-scoped dependency (this section is prescriptive; narrowing this edge requires a re-ratified revision, not a status-accuracy pass):** HYP-271's SaaS/browser slice already shipped (PR #270/`main`) without waiting on A1, but the row's own [§14.4](#144-acceptance-gate--erroredge-case-matrix) swallowed-prop entry still treats blind writes as unsafe with no realm carve-out, so the `all → A1` edge stands until that entry is itself revisited under a ratified revision | 11.1-11.3, 9.5          |
...
resolution (Phase 3) are the two most-asked-for capabilities. The SaaS/browser side of multi-select
is now merged and functional ([#270](https://github.com/hyperide/hyper-saas/pull/270), `main`), so the temptation this note guards against is narrower
than it once was — the standing temptation now is to ship **ext parity** ([HYP-427](https://linear.app/glide-vc/issue/HYP-427)) or the **full
stylability ladder** ahead of A1 + B1, or Tier-2 color resolution ahead of them, on top of the
ext-host's garbage-facts read. The map refuses this. Shipping ext-realm batch write or Tier-2
resolution before A1 + B1 exist means shipping a feature that writes targets it cannot verify and
routes on facts it knows are wrong — the exact failure class (D1/D3/D14) the master spec exists to
retire. The dependency edges are not suggestions; they are the difference between widening the
engine and widening the hole.
...
| SaaS i18n key read                                                                                             | **PARTIAL for Docker SaaS, WORKS for NodePod** ([HYP-372](https://linear.app/glide-vc/issue/HYP-372) Done, PR #477/#478 — no longer BROKEN/MISSING, but Docker SaaS is capability-narrower, see [§3.13](#313-cross-realm-transport-today)) | (shipped) `_tryDetectI18n` ported to a SaaS route + keys route + browser path wired; Docker existing-key-retarget-only capability gap is untickered — [HYP-746](https://linear.app/glide-vc/issue/HYP-746)'s scope is residual hardening only (durability, SW-intercept), not this gap | [HYP-746](https://linear.app/glide-vc/issue/HYP-746) (hardening only) + untickered (Docker create/dictionary parity), Backlog |
