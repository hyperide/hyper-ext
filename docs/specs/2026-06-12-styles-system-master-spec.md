...
cross-file, cross-system, one-`writeId` transaction. `lib/style-write/runtime-verify/` and the
attribution layer are absent on main (AS-IS [§9](#part-9--to-be-verify--transaction--undo) roll-up: "Runtime-verify + rollback transaction —
PLANNED"; do NOT cite a `lib/style-attribution/` path as existing — it does not exist on main, per
D19). The B0 saga lands in NEW PLANNED modules: `lib/style-write/runtime-verify/` (the B1 verify)
plus a `lib/style-write/transaction/` (the `writeId` journal + `rollback` API) — and the **D19 guard
holds**: `lib/style-attribution/` is NOT that module and must not be cited as existing. The B0
contract is the unification: realms differ ONLY in the thin FS transport beneath one shared `lib/`
`writeId`/`rollback` API (server FS snapshot, OPFS-pod, `vscode-file-io`); the transaction semantics
live in `lib/`, not in either host. Source: Q3 transaction-and-undo, D19 (stale-path guard).
...
RPC transport row and the authoritative verdict). AS-IS [§9](#part-9--to-be-verify--transaction--undo): "Runtime-verify (did the write land) +
rollback transaction — PLANNED."
...
write-ahead journal + hash-rechecked undo). No `writeId`-scoped journal exists on main.
