...
interface FillStyle {
  ...
  from?: Point; to?: Point;  // for linear
  center?: Point; radius?: number; // for radial
}
...
interface VectorSegment {
  start: number; end: number;           // vertex indices
  tangentStart: Point; tangentEnd: Point; // bezier handles (0,0 = straight)
}
...
interface ReconciliationDiff {
  ...
  modified: {
    params: Array<{               // param values changed
      nodeId: string;
      changes: Record<string, { old: unknown; new: unknown }>;
    }>;
    reordered: Array<{            // edges rewired (node moved in pipeline)
      edgeId: string;
      old: { source: string; target: string };
      new: { source: string; target: string };
    }>;
    muted: {                      // mute state toggled
