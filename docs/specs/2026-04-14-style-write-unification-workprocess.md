...
## 📍 Run #21 Checkpoint 5 (2026-04-28 ~18:40 CEST, ~2h13m)

| Shard | Done | Fail | Notes |
|-------|------|------|-------|
| s1    | 367  |   0  | error-state tests |
| s2    | 438  |   1  | style-editing TW tests |
| s3    | 283  |   8  | **2 new 605s timeouts** for remix-cssmodules-spotify |

### New failure type: `remix-cssmodules-spotify` — `PlayerBar` missing from `__canvas_preview__` registry

**Failing tests (S3):**
- `nested components — multiple selectors found` 604997ms — failed
- `ExportNamedDeclaration — correct traversal order` 605117ms — failed
- _(5 more ast-operations tests coming for same project)_

**Root cause analysis:**
1. `setup-preview.test.ts` maps `remix-cssmodules-spotify` → `app/components/PlayerBar.tsx` as entry component
2. `src/__canvas_preview__.tsx` (gitignored, on disk) had `Sidebar`, `AlbumRoute`, `LibraryRoute` — but NOT `PlayerBar`
3. Remix app renders `hydrateRoot(document, ...)` — no `<div id="root">` — but `isPreviewLoaded()` has body fallback
4. When `PlayerBar` is in `__canvas_preview__`, preview shows "Error: Component not found" (renders `h2` with "Error:" text)
5. `isPreviewLoaded()` detects the error `h2` → returns false for 600s → test timeout

**Why `PlayerBar` was missing**: The file on disk was generated by the extension from previous manual sessions that never opened `PlayerBar.tsx`. The extension would add it when called, but the test polls `isPreviewLoaded()` which detects the "Component not found" error before the extension can regenerate.

Actually — the real fix needed: the extension's `ensureComponent` is called AFTER the preview starts loading. There's a race between the extension adding `PlayerBar` to the registry and the preview's first render. If the first render sees `PlayerBar` missing → shows error → `isPreviewLoaded()` keeps returning false (error h2 found) → never transitions to loaded state.

**Fix applied**: Force-committed `src/__canvas_preview__.tsx` with `PlayerBar` in the registry (commit `b9b9f84`, pushed). Now Docker rsync ships the file with `PlayerBar`, and the preview loads correctly on first render.

**Impact on Run #21**: 5 more 600s timeouts expected for `remix-cssmodules-spotify` in S3 (~50 minutes). S1/S2 not affected (remix-cssmodules-spotify tests are in S3 only).

**Fix inventory for Run #22 (updated):**
1. error-overlay timeout 90→120s (S2 react-vite-tw3-kanban) — committed
2. element visibility 10→20s (S3 tamagui food-delivery/whatsapp) — committed
3. App Shell boundingBox poll 30s (S3 tamagui food-delivery/whatsapp) — committed
