/**
 * @file Add Point path operation — inserts a new anchor on a segment at parameter t
 *
 * Accessed via: Path menu > Add Anchor Point
 *
 * Assumptions: segmentIndex counts only drawable segments (Line, Cubic, Quad),
 *   not Move or Close commands. Arc segments are passed through unchanged.
 *   The Move starting the subpath is always preserved.
 *
...
export const addPointNode: NodeTypeDefinition = {
  type: 'addPoint',
  label: 'Add Point',
  category: 'pathOp',
  ...
  execute(inputs, params) {
    ...
    // Collect drawable segments (Line, Cubic, Quad) with their command index and start point.
