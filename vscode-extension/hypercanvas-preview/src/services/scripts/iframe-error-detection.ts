...
function checkErrors(): void {
  ...
  if (nextPortal?.shadowRoot) {
    const overlay = nextPortal.shadowRoot.querySelector('[data-nextjs-dialog-overlay]');
    if (overlay) {
      const body = nextPortal.shadowRoot.querySelector('[data-nextjs-dialog-body]');
      const header = nextPortal.shadowRoot.querySelector('[data-nextjs-dialog-header]');
      error = {
        framework: 'nextjs',
        type: header ? (header.textContent ?? '').trim() : 'Build Error',
        message: body ? (body.textContent ?? '').trim().substring(0, 500) : 'Unknown error',
        fullText: (overlay.textContent ?? '').trim().substring(0, 2000),
      };
      const fileEl = nextPortal.shadowRoot.querySelector('[data-nextjs-codeframe] p');
      if (fileEl) {
        const match = (fileEl.textContent ?? '').match(/(.+?):(\d+)/);
        if (match) {
          error.file = match[1];
          error.line = Number.parseInt(match[2], 10);
        }
      }
      const codeEl = nextPortal.shadowRoot.querySelector('[data-nextjs-codeframe] pre');
      if (codeEl) {
  ...
  if (!error) {
    ...
    if (viteOverlay?.shadowRoot) {
      const msgEl = viteOverlay.shadowRoot.querySelector('.message-body');
      ...
      if (msgEl) {
        error = {
          ...
          fullText: (viteOverlay.shadowRoot.textContent ?? '').trim().substring(0, 2000),
        };
        if (fileEl) {
          const match = (fileEl.textContent ?? '').match(/(.+?):(\d+)/);
          if (match) {
        ...
        if (frameEl) {
          error.codeframe = (frameEl.textContent ?? '').substring(0, 1000);
        }
      }
    }
  }
  ...
  if (!error) {
    ...
    if (bunOverlay?.shadowRoot) {
      const contentEl = bunOverlay.shadowRoot.querySelector('.error-content');
      if (contentEl) {
        error = {
          framework: 'bun',
          type: 'Build Error',
          message: (contentEl.textContent ?? '').trim().substring(0, 500),
          fullText: (bunOverlay.shadowRoot.textContent ?? '').trim().substring(0, 2000),
        };
      }
    }
  }
