...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  if (msg.type === 'hypercanvas:stateUpdate') {
    ...
    if (msg.writeInProgress !== undefined) {
      // Reset frozen rects on new write so stale rects from a previous selection
      // don't bleed into the new write's freeze window.
      if (msg.writeInProgress !== null && state.writeInProgress === null) {
