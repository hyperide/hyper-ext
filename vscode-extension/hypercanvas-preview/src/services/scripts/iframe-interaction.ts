...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  if (msg.type === 'hypercanvas:stateUpdate') {
    if (msg.selectedIds !== undefined) {
      const prevSelectedIds = state.selectedIds;
      state.selectedIds = msg.selectedIds;
      if (msg.selectedIds.length === 0) {
        // User explicitly deselected — don't keep showing ghost selection overlay.
        frozenSelectionRects = null;
        // Keep frozenWriteStartedAt alive while a write is in flight: if the user
        // re-selects something before HMR lands, the new frozen rects need the timer
        // to survive the HMR gap. Mirror the same guard used for A→B swaps below.
        if (!state.writeInProgress) {
          frozenWriteStartedAt = null;
        }
      } else if (
        frozenSelectionRects !== null &&
        (msg.selectedIds.length !== prevSelectedIds.length ||
          msg.selectedIds.some((id: string, i: number) => id !== prevSelectedIds[i]))
      ) {
