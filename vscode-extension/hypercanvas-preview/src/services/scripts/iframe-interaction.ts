...
const state = {
  ...
  writeInProgress: null as { writeId: string; startedAt: number } | null,
};
...
// === Write freeze state ===
// Keeps the last known selection rects visible during HMR rebuild after i18n writes.
// Freeze ends when: (a) fresh DOM match is found, or (b) WRITE_FREEZE_MAX_MS elapsed.
// frozenWriteStartedAt is tracked independently from state.writeInProgress so the
// 1500ms safety net continues even after RightSidebar clears writeInProgress at 800ms.
const WRITE_FREEZE_MAX_MS = 1500;
let frozenSelectionRects: OverlayRect[] | null = null;
// Tracks which selectedIds produced frozenSelectionRects so the else-if below can
// distinguish a legitimate element navigation from a transient HMR selectedIds:[]
// followed by a restore dispatch of the same element (prevSelectedIds would be [] in
// that case, making the length-diff check fire incorrectly).
let frozenSelectionIds: string[] = [];
let frozenWriteStartedAt: number | null = null;
...
function sendOverlayRects(): void {
  ...
  // Write freeze: keep the selection outline visible while HMR rebuilds the DOM.
  // frozenSelectionRects may carry the old elementId (pre-write nodeRef); during the
  // ~200-600ms HMR gap the user cannot interact with resize handles, so the stale id
  // is safe.
  //
  // IMPORTANT: do NOT clear frozenWriteStartedAt while writeInProgress is still set.
  // The async write takes 10-100ms; the first several RAFs after writeInProgress is
  // broadcast will still see the pre-write DOM and find fresh selection rects. Clearing
  // frozenWriteStartedAt at that point disarms the freeze before HMR fires (race
  // condition). Only disarm when writeInProgress is null (write + 800ms settled).
  let effectiveOverlayRects = result.overlayRects;
  const freshSelectionRects = result.overlayRects.filter((r) => r.type === 'selection');
  if (freshSelectionRects.length > 0) {
    // Real DOM match — update frozen state so next freeze has the latest position.
    frozenSelectionRects = freshSelectionRects;
    frozenSelectionIds = [...state.selectedIds];
    // Only clear the freeze timer once the write is done; while write is in progress
    // these rects may still be from the pre-write DOM (HMR hasn't fired yet).
    if (!state.writeInProgress) {
      frozenWriteStartedAt = null;
    }
  } else if (frozenSelectionRects !== null && frozenWriteStartedAt !== null) {
    const elapsed = Date.now() - frozenWriteStartedAt;
    if (elapsed < WRITE_FREEZE_MAX_MS) {
      // DOM is still rebuilding: inject last known rects so overlay stays visible.
      const nonSelectionRects = result.overlayRects.filter((r) => r.type !== 'selection');
      effectiveOverlayRects = [...nonSelectionRects, ...frozenSelectionRects];
      needsOverlayUpdate = true; // keep polling until new DOM element appears
    } else {
      frozenSelectionRects = null; // hard timeout — stop freezing
      frozenWriteStartedAt = null;
    }
  }

  const rects = effectiveOverlayRects.map((r) => ({
    key: r.key,
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  if (msg.type === 'hypercanvas:stateUpdate') {
    if (msg.selectedIds !== undefined) {
      ...
      if (msg.selectedIds.length === 0) {
        // During an active write, HMR may transiently emit selectedIds:[] as the fiber
        // tree rebuilds. Keep frozen rects so the overlay survives the HMR gap;
        // RightSidebar will re-broadcast the correct selection once the new DOM is ready.
        if (!state.writeInProgress) {
          frozenSelectionRects = null;
          frozenWriteStartedAt = null;
        } else {
          console.warn('[HyperCanvas] Selection cleared during write — freeze active, RightSidebar will restore');
        }
      } else if (
        frozenSelectionRects !== null &&
        (msg.selectedIds.length !== frozenSelectionIds.length ||
          msg.selectedIds.some((id: string, i: number) => id !== frozenSelectionIds[i]))
      ) {
        // Selection changed to a different element — frozen rects belong to the
        // previous selection and would show the wrong outline on the new element.
        // Compare against frozenSelectionIds (not prevSelectedIds) so that a transient
        // HMR selectedIds:[] followed by a restore dispatch of the same element does
        // not incorrectly clear the freeze (prevSelectedIds is [] at that point).
        frozenSelectionRects = null;
        // Preserve frozenWriteStartedAt if a write is still in flight: the new
        // selection needs freeze protection during the same HMR gap.
        // Clearing it here would violate the invariant documented at the freeze
        // block above ("do NOT clear frozenWriteStartedAt while writeInProgress
        // is still set") and leaves the new selection unprotected.
        if (!state.writeInProgress) {
          frozenWriteStartedAt = null;
        }
      }
    ...
    if (msg.writeInProgress !== undefined) {
      if (msg.writeInProgress !== null && msg.writeInProgress.writeId !== state.writeInProgress?.writeId) {
        // New write started — record expiry timestamp independently of the writeInProgress flag.
        // Do NOT reset frozenSelectionRects: keep existing frozen rects as the baseline for
        // the new freeze window (handles back-to-back writes while HMR is still in flight).
        frozenWriteStartedAt = msg.writeInProgress.startedAt;
      }
      state.writeInProgress = msg.writeInProgress;
    }
    needsOverlayUpdate = true;
    scheduleOverlayLoopIfNeeded();
