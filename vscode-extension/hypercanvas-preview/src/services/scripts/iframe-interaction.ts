...
function findElementsByRef(nodeRef: string, itemIndex: number | null): HTMLElement[] {
  ...
  if (live.length === 0 && source.fileName) {
    const closest = getSourceIndex().findClosestSourceDOMElements(source);
    if (closest !== null && closest.elements.length > 0) {
      live = closest.elements;
      const matchedKey = `${closest.matchedSource.fileName}:${closest.matchedSource.line}:${closest.matchedSource.column}`;
      logSelsurvClosestSourceFallback(nodeRef, matchedKey, live.length);
    }
  }

  // Cross-format last-resort: tree-driven selection dispatches an absolute path
  // ("/workspace/src/Foo.tsx") or Windows-style ("C:\\workspace\\src\\Foo.tsx")
  // while FiberSourceIndex stores Vite-relative paths ("src/Foo.tsx"). Path-suffix
  // matching covers both the exact-line case AND the HMR line-shift case safely —
  // a filename-agnostic line:col scan would mis-anchor the selection to an unrelated
  // file that happens to share the same line:col.
  if (live.length === 0 && source.fileName) {
    const closest = getSourceIndex().findClosestSourceDOMElements(source, { matchPathAcrossFormats: true });
    if (closest !== null && closest.elements.length > 0) {
      live = closest.elements;
      const matchedKey = `${closest.matchedSource.fileName}:${closest.matchedSource.line}:${closest.matchedSource.column}`;
      logSelsurvClosestSourceFallback(`${nodeRef}#xfmt`, matchedKey, live.length);
    }
  }
