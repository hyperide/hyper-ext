...
function sendOverlayRects(): void {
  ...
  // is safe. The freeze ends as soon as a fresh DOM match arrives (branch 1 below).
  ...
  } else if (state.writeInProgress !== null && frozenSelectionRects !== null) {
    const elapsed = Date.now() - state.writeInProgress.startedAt;
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  if (msg.type === 'hypercanvas:stateUpdate') {
    if (msg.selectedIds !== undefined) {
      // Note: selection may legitimately clear during a write (user clicks away).
    ...
    if (msg.writeInProgress !== undefined) {
      // Reset frozen rects on any new write (including write-over-write) so stale
      // rects from the previous selection don't bleed into the new freeze window.
      if (
        msg.writeInProgress !== null &&
        msg.writeInProgress.writeId !== state.writeInProgress?.writeId
      ) {
