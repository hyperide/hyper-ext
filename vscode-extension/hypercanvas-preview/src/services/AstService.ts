...
export class AstService {
  ...
  private async _moveAcrossFiles(opts: {
    ...
    for (const name of refs) {
      ...
      if (!found) {
        // Skip names that are obviously not user-defined references (host tag
        // roots already excluded by collectJsxExternalRefs; React globals,
        // arrow params survive). Heuristic: only complain about PascalCase
        // names (likely components / classes) that we couldn't resolve. Other
        // names (`event`, `index`, `e`) are loop variables / event handlers
        // that don't need an import.
        if (/^[A-Z]/.test(name)) {
