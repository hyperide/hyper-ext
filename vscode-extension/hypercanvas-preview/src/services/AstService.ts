...
export class AstService {
   ...
   *
   * Implements the contract documented above (`MoveResult`). Task 2 scope:
   * same-file moves only — both same-JSX-parent (sibling reorder) and
   * different-JSX-parent (cut from old parent, splice at target). Cross-file
   * support is added in Task 3 by following each NodeRef to its actual
   * source file via `_resolveElementInCorrectFile`.
   *
  ...
  async moveElement(
    ...
    // Task 2 scope: same-file only. Cross-file (Task 3) needs cut-and-paste
    // across two ASTs plus import bookkeeping — that lands in the next
    // iteration. Surface a clear "not yet implemented" error here so
    // callers don't silently skip the write.
    if (sourceLocate.resolvedPath !== targetLocate.resolvedPath) {
      throw new Error(
        `moveElement: cross-file moves not implemented yet (source=${sourceLocate.resolvedPath}, target=${targetLocate.resolvedPath})`,
      );
    }
    ...
    if (sourceParent === targetParent) {
      // Same-parent: array-level reorder, identical to reorderElement.
