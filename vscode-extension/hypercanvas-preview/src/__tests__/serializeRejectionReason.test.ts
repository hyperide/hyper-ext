...
describe('isForeignExtensionError', () => {
  ...
  it('returns false when both HyperIDE and foreign extension frames appear in the stack', () => {
    // Mixed stack: foreign frame appears first but our code is also present.
    // Should be treated as ours so diagnostic data is not silently dropped.
    const err = new Error('mixed');
    err.stack = [
      'Error: mixed',
      '    at /Users/user/.vscode/extensions/github.copilot-1.0/dist/ext.js:1:1',
      '    at /Users/user/.vscode/extensions/hyperide.hypercanvas-0.1.8/out/extension.js:100:5',
    ].join('\n');
    expect(isForeignExtensionError(err)).toBe(false);
  });
});
