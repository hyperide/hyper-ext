...
`;
  ...
  it('surfaces lowercase props/hooks/helpers as unresolved-ref adjustments instead of silently emitting broken target code', async () => {
    // Cross-file move where the moved subtree references a hook return value
    // (`elapsed`), a helper bound in the source-component scope (`formatMs`),
    // and a prop (`label`) — none of which are imports. The previous PascalCase
    // filter dropped these silently; the relaxed heuristic must surface them
    // so the user knows the target file won't build until they fix the binds
    // up. Arrow params declared INSIDE the subtree (e.g. the `.map((item) => …)`
    // callback) must NOT be flagged — they travel with the moved JSX.
    const sourceFile = `import { useTimer } from './hooks/useTimer';

export function Source({ label }: { label: string }) {
  const elapsed = useTimer();
  function formatMs(n: number): string { return String(n); }
  const items = ['a', 'b'];
  return (
    <div className="src-root">
      <span className="moved">{label}: {formatMs(elapsed)} {items.map((item) => <em key={item}>{item}</em>)}</span>
    </div>
  );
}
`;
    const targetFile = `export function Target() {
  return (
...
`;
    ...
    const sourceRef = refByClass(service, '/workspace/src/Source.tsx', 'src/Source.tsx', 'span', 'moved', sourceFile);
    const targetRef = refByClass(service, '/workspace/src/Target.tsx', 'src/Target.tsx', 'h1', 'tgt-title', targetFile);

    const result = await service.moveElement('src/Source.tsx', sourceRef, targetRef, 'after');
    expect(result.success).toBe(true);

    // The unresolved-refs adjustment must mention `label` (prop), `formatMs`
    // (source-scope helper), `elapsed` (hook return), AND `items` (outer
    // array bound at component scope, not inside the moved subtree). It must
    // NOT mention `item` (the .map callback param declared INSIDE the moved
    // subtree) or `useTimer` (resolved as an import and replicated separately).
    const unresolved = result.adjustments?.find((a) => a.includes('unresolved references in moved subtree'));
    expect(unresolved).toBeDefined();
    expect(unresolved).toContain('label');
    expect(unresolved).toContain('formatMs');
    expect(unresolved).toContain('elapsed');
    expect(unresolved).toContain('items');
    // Match against comma-separated list to avoid `items` substring noise.
    const namesPart = unresolved?.split(': ')[1]?.split(' — ')[0] ?? '';
    const names = namesPart.split(', ');
    expect(names).not.toContain('item');
    expect(names).not.toContain('useTimer');
  });
});
