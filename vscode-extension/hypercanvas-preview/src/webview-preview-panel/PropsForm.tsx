...
export function PropsForm({
  ...
  // Auto-generate values on first render when required fields exist and form is empty.
  // Uses field count to detect when schema arrives (fields.length transitions from 0 → N).
  const autoGeneratedForCountRef = useRef(0);
  useEffect(() => {
    if (fields.length === 0) return;
    if (autoGeneratedForCountRef.current === fields.length) return;
    const hasRequired = fields.some((f) => f.typeInfo.required);
    const allEmpty = Object.keys(values).length === 0;
    if (hasRequired && allEmpty) {
      autoGeneratedForCountRef.current = fields.length;
      const generated = generateObjectValues(fields);
      setValues(generated);
      onChange(generated);
    }
