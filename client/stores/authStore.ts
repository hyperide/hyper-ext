...
export const useAuthStore = create<AuthState>()(
  ...
  setWorkspaces: (workspaces) => {
    ...
    if (!current && workspaces.length > 0) {
      ...
      const savedWorkspace = savedSlug
  ...
  checkAuth: async () => {
    ...
    try {
      ...
      if (!userResponse.ok && userResponse.status === 401) {
        ...
        console.log("[Auth checkAuth] /refresh result:", retryRefreshResult.ok ? 'ok' : (retryRefreshResult as RefreshError).code);
        ...
        if (retryRefreshResult.ok) {
          ...
          console.log("[Auth checkAuth] Refresh failed with code:", failedRetry.code, ", wasLoggedIn:", !!wasLoggedIn);
          ...
          if (wasLoggedIn && isNetworkOrServerError) {
            ...
            console.log("[Auth checkAuth] Soft error on 401-refresh (network/server down), setting connectionError");
