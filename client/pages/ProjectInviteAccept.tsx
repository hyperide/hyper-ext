...
export default function ProjectInviteAccept() {
	...
	useEffect(() => {
		...
		const acceptInvite = async () => {
			...
			if (!isAuthenticated || !accessToken || !pendingToken || accepting)
				return;
			...
			try {
				const response = await authFetch(
					`/api/project-invites/${pendingToken}/accept`,
					{
						method: 'POST',
					}
				);
					...
					<CardDescription>
						{inviteInfo ? (
							<>
								...
								<strong>{inviteInfo.projectName}</strong> as {roleLabel}
								{inviteInfo.inviterName && (
									<> by {inviteInfo.inviterName}</>
								)}
							</>
						) : (
					...
					{success && (
						<div className="p-3 mb-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
							<p className="text-sm text-green-600 dark:text-green-400">
								Successfully joined project! Redirecting...
							</p>
						</div>
					)}
