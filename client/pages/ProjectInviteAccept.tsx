...
export default function ProjectInviteAccept() {
  ...
  useEffect(() => {
    const fetchInviteInfo = async () => {
      ...
      try {
        const response = await fetch(`/api/project-invites/${token}/info`);
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    if (token) {
      sessionStorage.setItem('pending_project_invite', token);
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    const acceptInvite = async () => {
      const pendingToken = sessionStorage.getItem('pending_project_invite');
      ...
      try {
        const response = await authFetch(`/api/project-invites/${pendingToken}/accept`, {
          method: 'POST',
        });
        ...
        sessionStorage.removeItem('pending_project_invite');
  ...
  }, [isAuthenticated, accessToken, accepting, navigate]);

  const handleLogin = () => {
    // Store redirect for after login
    sessionStorage.setItem('auth_redirect', `/project-invite/${token}`);
  ...
  const roleLabel = inviteInfo?.role === 'editor' ? 'Editor' : 'Viewer';
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            {inviteInfo ? (
              <>
                You have been invited to join <strong>{inviteInfo.projectName}</strong> as {roleLabel}
        ...
        <CardContent>
          ...
          {success && (
            <div className="p-3 mb-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
              <p className="text-sm text-green-600 dark:text-green-400">Successfully joined project! Redirecting...</p>
            </div>
          )}
