...
import {
	...
	InsertInstancePanel,
} from '@/components/FloatingPanels';
...
import { useProjectControl, type ProjectData } from './components/hooks/useProjectControl';
...
export function CanvasEditor({ onOpenSettings }: Props) {
	...
	} = useCanvasResizeHandlers();

	const [editingCondBoundary, setEditingCondBoundary] =
		useState<CondBoundary | null>(null);
	const [editingMapBoundary, setEditingMapBoundary] =
		useState<MapBoundary | null>(null);
	...
	const [iframeError, setIframeError] = useState<{ message: string | null; retryCount: number }>({ message: null, retryCount: 0 });
	...
	const { projectUIKit, activeProjectId, activeProjectName, publicDirExists, configError: projectConfigError } = useProjectUIKit(activeProject);
	...
	const [activeDesignInstanceId, setActiveDesignInstanceId] = useState<
		string | null
	>(null);
	const [activeBoardInstance, setActiveBoardInstance] = useState<string | null>(
		null,
	);
	...
	const [mode, setMode] = useState<'design' | 'interact' | 'code'>(
		engine.getMode(),
	);
	...
	const { iframeLoadedCounter, instancesReadyCounter, triggerIframeReload } =
		useIframeLoadTracking({
			enabled:
				!!activeProject &&
	...
	const documentTitle =
		isCodeEditorMode && activeFilePath
			? activeFilePath.split('/').pop() || null
			: meta?.componentName || null;
		...
		const handleModeChange = ({
			...
			if (mode === 'interact') {
				authFetch('/api/clear-classname-cache', {
					...
					console.error(
						'[CanvasEditor] Failed to clear className cache:',
						error,
					);
				});
			}
	...
	const resetZoomToTopLeftInstance = useCallback(() => {
		...
		const instanceElements = iframe.contentDocument.querySelectorAll(
	...
	const handleFitToContent = useCallback(() => {
		...
		// Center content
		const newPanX =
			(containerRect.width - contentWidth * newZoom) / 2 - minX * newZoom;
		const newPanY =
			(containerRect.height - contentHeight * newZoom) / 2 - minY * newZoom;
	...
	const handleToolbarModeChange = useCallback(
		(newMode: Tool) => {
			...
			if (isCodeEditorMode || isBoardModeActive) {
				...
				const iframe = getPreviewIframe();
				if (iframe?.contentDocument) {
					const iframeInstances = iframe.contentDocument.querySelectorAll(
						'[data-canvas-instance-id]',
					);
					if (iframeInstances.length > 0) {
						const firstInstanceId = (iframeInstances[0] as HTMLElement).dataset
							.canvasInstanceId;
						if (firstInstanceId) {
		...
		>
			...
			>
				...
				{!sidebarsHidden && !isCodeEditorMode && (
					<>
						<div
							data-uniq-id="6d51b678-1b74-45c1-9612-769b13c121dd"
				...
				{/* Canvas Area */}
				<div
					data-uniq-id="bf326e7f-4bb4-4243-b74f-dd07fedc06b3"
				...
				>
						...
						>
								...
								{projectConfigError && !configErrorDismissed ? (
									...
									availableComponents.isLoaded &&
									  ...
									  availableComponents.composites.length === 0 ? (
										...
										<>
											<div
												style={{
													...
													paddingTop:
														canvasMode === 'single' ? '26px' : undefined,
										...
										>
											<div
												data-uniq-id="b4549492-9d26-45de-9be0-b717b0564794"
										...
										>
											<div
												data-uniq-id="def01234-5678-9abc-def0-123456789abc"
											...
											>
												...
												/>
												<p
													data-uniq-id="897d4992-52e1-40b3-b386-39746f9ade79"
							...
							<div
								...
								style={{
									display:
										isCodeEditorMode || isBoardModeActive ? 'none' : undefined,
								}}
							/>
							...
							<div
								data-uniq-id="619bbf5f-ff78-4979-a95d-1cfb079d3ac4"
								...
								portalContainerRef.current && (
									<MapEditPopup
										...
										instanceId={
											canvasMode === 'single'
												? 'default'
												: activeDesignInstanceId || Object.keys(instances)[0]
										}
										onItemsGenerated={() => {
							...
							<InstanceEditPopup
								...
								componentName={meta?.componentName}
								instanceConfig={
									editingInstanceId ? instances[editingInstanceId] : undefined
								}
						...
						{!isCodeEditorMode && (hasGatewayError || runtimeError || parseErrorAsRuntimeError) && activeProject?.id && (
							<LogsPanel
								projectId={activeProject.id}
								...
								onClose={handleGatewayErrorClose}
							/>
						)}
						</div>
				</div>
				...
				{!isCodeEditorMode && !sidebarsHidden && (
					...
					>
						<div className="flex-1 flex flex-col h-full">
							{isAIChatDocked && isAIChatOpen ? (
								// Spacer content is empty — AI chat renders as fixed overlay
								null
							) : (
            ...
            <AlertDialog
				open={!!pendingSizeChange}
				onOpenChange={(open) => !open && setPendingSizeChange(null)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>Change viewport size?</AlertDialogTitle>
						<AlertDialogDescription>
							You have comments on this component. Changing the viewport size
							may cause comments to appear in unexpected positions.
							<br />
							<br />
							<strong>Tip:</strong> Instead of changing size, consider adding
							another instance with different dimensions to compare layouts.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						...
						<AlertDialogAction
							onClick={() => {
								if (pendingSizeChange) {
									applyInstanceSizeChange(
										pendingSizeChange.width,
										pendingSizeChange.height,
									);
