...
export function useInstanceInteraction({
	...
	const handleInstanceDoubleClick = useCallback(
		(instanceId: string) => {
			console.log(
				'[Double Click] Start transition to design mode:',
				instanceId,
			);
	...
	const handleInstanceBadgeClick = useCallback(
		...
		[mode, activeDesignInstanceId, isBoardModeActive, setEditingInstanceId, setEditPopupOpen, setActiveDesignInstanceId],
	);

	// Click on another instance while staying in current mode
	const handleOtherInstanceClick = useCallback((instanceId: string) => {
		setActiveDesignInstanceId(instanceId);
	...
	}, [canvasMode, selectedCommentId, setSelectedCommentId, setActiveDesignInstanceId, setActiveBoardInstance, setBoardModeActive]);
