	...
	useEffect(() => {
		...
		const loadComponents = async () => {
			try {
				...
				const atoms =
					data.atomGroups?.flatMap(
						(group: { components: ComponentInfo[] }) => group.components,
					) || [];
				const composites =
					data.compositeGroups?.flatMap(
						(group: { components: ComponentInfo[] }) => group.components,
					) || [];
				...
				if (persistedState.openedComponent && !currentComponentName) {
					...
					const persistedComponent = allComponents.find(
						(comp) => comp.path === persistedState.openedComponent,
					);

					if (persistedComponent) {
						console.log(
							'[useComponentAutoLoad] Restoring previously opened component:',
							persistedComponent.name,
						);
				...
				if (!componentLoaded && mode !== 'code') {
					const currentIsEntryPoint =
						currentComponentName && isEntryPoint(currentComponentName);

					if (!currentComponentName || currentIsEntryPoint) {
						if (currentIsEntryPoint) {
							console.log(
								'[useComponentAutoLoad] Current component is entry point, auto-selecting first available',
							);
						}
						...
						if (firstComposite) {
							console.log(
								'[useComponentAutoLoad] Auto-loading first composite:',
								firstComposite.name,
							);
							loadComponent(firstComposite.path);
						} else if (firstAtom) {
							console.log(
								'[useComponentAutoLoad] Auto-loading first atom:',
								firstAtom.name,
							);
