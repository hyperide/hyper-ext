...
export function useInstancePositioning({
	...
	const handleInstanceDragEnd = useCallback(
		(instanceId: string, deltaX: number, deltaY: number) => {
			...
			const stickers = document.querySelectorAll(
				`[data-comment-instance-id="${instanceId}"]`,
			);
	...
	const handleInstanceDragging = useCallback(
		(instanceId: string | null, deltaX: number, deltaY: number) => {
			if (instanceId) {
				...
				const saveSize = async () => {
					try {
						...
						const allInstanceIds = new Set([
							...currentDomInstanceIds,
							...(canvasMode === 'single' ? ['default'] : []),
						]);
						...
						console.error(
							'[useInstancePositioning] Failed to save instance size:',
							error,
						);
					}
				};
	...
	const handleInstanceSizeChange = useCallback(
		(width: number, height: number) => {
			...
			const hasComments =
				canvasMode === 'single' &&
				comments.some((c) => !c.parentId && c.status !== 'resolved');
	...
	useEffect(() => {
		...
		setInstances((prev) => {
			...
			const templateInstance = Object.values(prev).find(
