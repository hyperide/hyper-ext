...
import { migrateExcalidrawAnnotations, needsMigration } from '@/lib/annotations-utils';
...
export function useCanvasComposition({
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.composition) {
          ...
          console.log('[useCanvasComposition] Setting annotations:', newAnnotations?.length);
  ...
  useEffect(() => {
    const handleCanvasChanged = () => {
      console.log('[useCanvasComposition] CustomEvent canvasCompositionChanged received');
      reloadComposition();
    };
