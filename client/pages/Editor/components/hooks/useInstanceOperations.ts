  ...
  const handleInstancePaste = useCallback(async () => {
    ...
    try {
      ...
      if (!code || !code.includes('return')) {
        console.warn('[Instance Paste] Clipboard does not contain valid renderer code');
        return;
      }
      ...
      if (!listData.success) {
        console.error('[Instance Paste] Failed to get renderer list:', listData);
        return;
      }
    ...
    async (instanceId: string) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          iframe?.contentDocument?.querySelector(`[data-canvas-instance-id="${instanceId}"]`)?.remove();
        } else {
