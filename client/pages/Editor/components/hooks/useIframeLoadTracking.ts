...
export function useIframeLoadTracking({
	...
	useEffect(() => {
		...
		if (!iframe) {
			console.log(
				'[useIframeLoadTracking] Iframe not found yet, will retry when mode/component changes',
			);
			return;
		}
		...
		const handleIframeLoad = () => {
			console.log(
				'[useIframeLoadTracking] Iframe loaded, incrementing counter',
				componentName,
			);
	...
	useEffect(() => {
		...
		const existingInstances = iframeDoc.querySelectorAll(
				...
				if (mutation.type === 'childList') {
					for (const node of mutation.addedNodes) {
						if (node instanceof HTMLElement) {
							if (
								node.hasAttribute('data-canvas-instance-id') ||
								node.querySelector('[data-canvas-instance-id]')
							) {
								setInstancesReadyCounter((prev) => prev + 1);
								observer.disconnect();
								return;
