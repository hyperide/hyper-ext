...
export function useCanvasComments({
  ...
  useEffect(() => {
    if (!activeProjectStatus || activeProjectStatus !== 'running' || isCodeEditorMode || mode !== 'design') {
      return;
    }
    ...
    const updateCommentPositions = () => {
      ...
      const pendingInput = document.querySelector('[data-pending-comment-input]') as HTMLElement;
      if (pendingInput) {
      ...
      if (dragging) {
        const arrowGroups = document.querySelectorAll('[data-annotation-type="arrow"]');
        ...
        for (const group of Array.from(arrowGroups)) {
          ...
          if (startBinding !== dragging.instanceId && endBinding !== dragging.instanceId) {
            continue;
          }
          ...
          const startX = startBinding === dragging.instanceId ? baseStartX + dragging.deltaX : baseStartX;
          const startY = startBinding === dragging.instanceId ? baseStartY + dragging.deltaY : baseStartY;
          const endX = endBinding === dragging.instanceId ? baseEndX + dragging.deltaX : baseEndX;
          const endY = endBinding === dragging.instanceId ? baseEndY + dragging.deltaY : baseEndY;
          ...
          const startHandle = g.querySelector('[data-arrow-handle$=":start"]') as SVGCircleElement;
          const endHandle = g.querySelector('[data-arrow-handle$=":end"]') as SVGCircleElement;
          if (startHandle) {
          ...
          if (selectionRect) {
            ...
            selectionRect.setAttribute('width', String(Math.abs(endX - startX) + 16));
            selectionRect.setAttribute('height', String(Math.abs(endY - startY) + 16));
          }
