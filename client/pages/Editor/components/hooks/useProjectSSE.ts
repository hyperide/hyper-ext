...
export function useProjectSSE({
    ...
    const doPoll = async () => {
      ...
      try {
        ...
        if (data.status === 'running') {
          ...
          setActiveProject((prev: ProjectData | null) => (prev ? { ...prev, status: 'running' } : prev));
        } else if (data.status === 'building') {
          console.log('[StatusPoll] Project is building, updating state');
          setActiveProject((prev: ProjectData | null) => (prev ? { ...prev, status: 'building' } : prev));
        }
      } catch (err) {
  ...
  useReconnectingEventSource({
    ...
    onMessage: useCallback(
      (data: unknown) => {
        ...
        console.log('[SSE] Project update:', project.name, 'status:', project.status);
  ...
  useEffect(() => {
    ...
    const fallbackTimeout = setTimeout(async () => {
      ...
      console.log('[SSE] Fallback: SSE did not deliver data, fetching via HTTP...');
      try {
        if (persistedProjectId) {
  ...
  useReconnectingEventSource({
    ...
    onMessage: useCallback(
      (data: unknown) => {
        ...
        if (event.type === 'components_updated' || event.type === 'git_status_changed') {
