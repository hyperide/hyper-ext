...
interface UseCommentHandlersReturn {
  ...
  handleAddComment: (position: { x: number; y: number }, elementId: string | null, instanceId: string | null) => void;
...
export function useCommentHandlers({
  ...
  const [pendingCommentPosition, setPendingCommentPosition] = useState<PendingCommentPosition | null>(null);

  // Show size selection dialog before adding comment (when size is Auto)
  const [showSizeSelectionForComment, setShowSizeSelectionForComment] = useState(false);

  // Handle adding comment on canvas click
  const handleAddComment = useCallback(
    (position: { x: number; y: number }, elementId: string | null, instanceId: string | null) => {
      console.log(
