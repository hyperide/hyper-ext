...
import { createArrowAnnotation, createTextAnnotation, detectBinding } from '../../lib/annotations-utils';
      ...
      for (const ann of annotations) {
        if (isArrowAnnotation(ann)) {
          ...
          if (selectedIds.includes(ann.id)) {
            ...
            const startDist = Math.sqrt((canvasX - ann.startX) ** 2 + (canvasY - ann.startY) ** 2);
            if (startDist <= handleRadius) {
              return { id: ann.id, endpoint: 'start' };
            }
            const endDist = Math.sqrt((canvasX - ann.endX) ** 2 + (canvasY - ann.endY) ** 2);
          ...
          // Check line hit area
          const hitWidth = 10;
          const dist = pointToLineDistance(canvasX, canvasY, ann.startX, ann.startY, ann.endX, ann.endY);
          if (dist <= hitWidth) {
            return { id: ann.id };
          }
        }
        ...
        if (isTextAnnotation(ann)) {
          ...
          if (canvasX >= ann.x && canvasX <= ann.x + width && canvasY >= ann.y && canvasY <= ann.y + height) {
            return { id: ann.id };
          }
        }
      }
    ...
    (marquee: MarqueeState) => {
      ...
      for (const ann of annotations) {
        if (isArrowAnnotation(ann)) {
          ...
          const lineIntersects = lineIntersectsRect(ann.startX, ann.startY, ann.endX, ann.endY, minX, minY, maxX, maxY);
  ...
  const handleMouseDown = useCallback(
    (e: MouseEvent) => {
      ...
      if (hit) {
        ...
        const willBeSelected = toggle ? !isAlreadySelected : true;
            ...
            if (isArrowAnnotation(initial) && isArrowAnnotation(ann)) {
              ...
              if (currentDrag.endpoint === 'start') {
                return {
                  ...
                  startX: initial.startX + deltaX,
                  startY: initial.startY + deltaY,
                  startBinding: detectBinding(initial.startX + deltaX, initial.startY + deltaY, instances),
                };
              }
              if (currentDrag.endpoint === 'end') {
                return {
                  ...
                  endX: initial.endX + deltaX,
                  endY: initial.endY + deltaY,
                  endBinding: detectBinding(initial.endX + deltaX, initial.endY + deltaY, instances),
                };
              }
            }
    ...
    (e: MouseEvent) => {
      ...
      if (drawingState) {
        ...
        if (distance > 10) {
          ...
          const newArrow = createArrowAnnotation(drawingState.startX, drawingState.startY, x, y, {
            ...
            onLabelChange={(label) => {
              if (operations) {
                ...
                const updated = annotations.map((a) =>
                  a.id === arrow.id && isArrowAnnotation(a) ? { ...a, label, version: a.version + 1 } : a,
                );
                onChange?.(updated);
              }
            }}
          ...
          <TextElement
            ...
            onChange={(updated) => {
              if (operations) {
                ...
                const newAnnotations = annotations.map((a) => (a.id === updated.id ? updated : a));
