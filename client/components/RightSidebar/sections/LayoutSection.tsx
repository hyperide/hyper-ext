			...
			<div className="flex items-center gap-1.5 mb-3">
				<div className="flex items-center gap-1.5 w-sidebar-content">
					<div className="flex-1 h-6 px-2 bg-muted rounded flex items-center gap-1">
						...
						<Input
							...
							onBlur={onWidthBlur}
							onKeyDown={(e) =>
								onNumericKeyDown(e, width, (v) => onWidthChange(v), 'width')
							}
							placeholder="auto"
							className={cn(
					...
					<div className="flex-1 h-6 px-2 bg-muted rounded flex items-center gap-1">
						...
						<Input
							...
							onBlur={onHeightBlur}
							onKeyDown={(e) =>
								onNumericKeyDown(e, height, (v) => onHeightChange(v), 'height')
							}
							placeholder="auto"
							className={cn(
								'h-auto border-0 bg-transparent !text-[11px] p-0 focus-visible:ring-0 focus-visible:ring-offset-0 flex-1',
				...
				>
					<IconAspectRatio
						className={cn(
							'w-4 h-4',
							aspectRatioLocked ? 'text-[#3479DE]' : 'text-foreground',
						)}
						stroke={1.5}
					/>
				</button>
					...
					<div className="flex items-start gap-1.5 mb-3">
						<div className="w-[97px] h-14 rounded-md bg-muted relative">
							<div className="absolute inset-0 grid grid-cols-3 grid-rows-3">
								{LAYOUT_OPTIONS.map((pos) => {
									...
									const isSpaceBetweenActive = isSpaceBetween && (
									...
									const isActive =
										(selectedLayout === 'row' &&
											normalizedJustify === pos.justify &&
											normalizedAlign === pos.align) ||
										(selectedLayout === 'col' &&
											normalizedJustify === pos.align &&
											normalizedAlign === pos.justify);
									return (
										<button
											key={`${pos.col}-${pos.row}`}
											type="button"
						...
						<div className="flex-1 h-6 px-2 bg-muted rounded flex items-center gap-1">
							...
							<Input
								...
								onKeyDown={(e) =>
									onNumericKeyDown(e, gap, onGapChange, 'gap')
								}
						...
						<button
							type="button"
							className="w-6 h-6 rounded flex items-center justify-center invisible"
						>
							<IconAdjustmentsHorizontal
								...
								<Input
									...
									onKeyDown={(e) =>
										onNumericKeyDown(
											e,
											paddingTop,
											(v) => onPaddingChange('paddingTop', v),
											'paddingTop',
										)
									}
									placeholder="0px"
								...
								<Input
									...
									onKeyDown={(e) =>
										onNumericKeyDown(
											e,
											paddingRight,
											(v) => onPaddingChange('paddingRight', v),
											'paddingRight',
										)
									}
									placeholder="0px"
								...
								<Input
									...
									onKeyDown={(e) =>
										onNumericKeyDown(
											e,
											paddingBottom,
											(v) => onPaddingChange('paddingBottom', v),
											'paddingBottom',
										)
									}
									placeholder="0px"
								...
								<Input
									...
									onKeyDown={(e) =>
										onNumericKeyDown(
											e,
											paddingLeft,
											(v) => onPaddingChange('paddingLeft', v),
											'paddingLeft',
										)
									}
									placeholder="0px"
							...
							<div className="absolute inset-0 grid grid-cols-3 grid-rows-3">
								{(() => {
									...
									return LAYOUT_OPTIONS.map((pos) => {
										...
										const alignValue =
											pos.align === 'flex-start'
												? 'start'
												: pos.align === 'flex-end'
													? 'end'
													: pos.align;
										const justifyValue =
											pos.justify === 'flex-start'
												? 'start'
												: pos.justify === 'flex-end'
													? 'end'
													: pos.justify;
										...
										const isActive =
											!isAnyStretch &&
											gridJustifyItems === pos.justify &&
											gridAlignItems === pos.align;
