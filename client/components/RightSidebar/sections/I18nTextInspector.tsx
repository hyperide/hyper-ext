...
export const I18nTextInspector = memo(function I18nTextInspector({
  ...
  const commitKey = (key: string) => {
    // Diagnostic logging for HYP-i18n-text-edit-disabled Task 3 — trace the
    // "Create key" flow: combobox button → onKeyChange → RightSidebar handler.
    // Remove once E2E coverage proves the path is reliable.
    console.debug('[I18nTextInspector] commitKey', {
      key,
      currentKey,
      hasOnKeyChange: typeof onKeyChange === 'function',
    });
