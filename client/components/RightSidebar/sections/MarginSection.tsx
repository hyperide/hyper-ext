...
export const MarginSection = memo(function MarginSection({
	...
	const handleHorizontalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		const result = updateSpacingFromInput(
			e.target.value,
			marginLeft,
			marginRight,
			marginLinked,
	...
	const handleHorizontalKeyDown = (
		...
		const result = handleSpacingArrowKey(
			displayValue,
			cursorPos,
			marginLeft,
			marginRight,
		...
		setTimeout(() => {
			const newValue = getSpacingDisplayValue(
				result.first,
				result.second,
				marginLinked,
			);
			...
			if (result.firstChanged) {
				input.setSelectionRange(
					0,
					commaIdx === -1 ? newValue.length : commaIdx,
				);
	...
	return (
		<div className="px-4 pb-3 max-w-sidebar-section overflow-hidden">
			...
			<div className="flex items-start gap-2 mb-2">
				<div className="grid grid-cols-2 gap-2 w-sidebar-content">
					<div className="h-6 px-2 bg-muted rounded flex items-center gap-1">
						<span className="text-xs text-muted-foreground">
							{marginLinked ? 'Left' : 'Hor'}
						</span>
						<Input
							type="text"
							value={getSpacingDisplayValue(
								marginLeft,
								marginRight,
								marginLinked,
							)}
						...
						<>
							<div className="h-6 px-2 bg-muted rounded flex items-center gap-1">
								...
								<Input
									...
									value={marginRight}
									onChange={(e) =>
										onMarginChange('marginRight', e.target.value)
									}
									onKeyDown={(e) =>
										onNumericKeyDown(
											e,
											marginRight,
											(v) => onMarginChange('marginRight', v),
											'marginRight',
										)
									}
									placeholder="0px"
							...
							<div className="h-6 px-2 bg-muted rounded flex items-center gap-1">
								...
								<Input
									...
									value={marginBottom}
									onChange={(e) =>
										onMarginChange('marginBottom', e.target.value)
									}
									onKeyDown={(e) =>
										onNumericKeyDown(
											e,
											marginBottom,
											(v) => onMarginChange('marginBottom', v),
											'marginBottom',
										)
									}
									placeholder="0px"
				...
				>
					<IconBorderSides
						className={`w-4 h-4 ${marginLinked ? 'text-[#3479DE]' : 'text-foreground'}`}
						stroke={1.5}
					/>
				</button>
			</div>
