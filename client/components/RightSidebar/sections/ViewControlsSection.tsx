				...
				>
					...
					<span className="text-xs font-medium text-foreground truncate">
						{instanceSize
							? (SIZE_PRESETS.find(
									(p) =>
										p.width === instanceSize.width &&
										p.height === instanceSize.height,
								)?.label ?? `${instanceSize.width}×${instanceSize.height}`)
							: 'Auto'}
					</span>
						...
						{SIZE_PRESETS.map((preset) => (
							<button
								...
								className={cn(
									...
									instanceSize?.width === preset.width &&
										instanceSize?.height === preset.height &&
										'bg-muted',
								)}
							>
