...
export default function Toolbar({
  ...
  const { isAddingComment, setIsAddingComment, isReadonly, isAIChatOpen, openAIChat, closeAIChat } = useEditorStore();
  ...
  const handleAddInstance = async () => {
    ...
    if (!meta?.projectId || !meta?.relativeFilePath) {
      console.error('[Toolbar] Cannot add instance: missing project or component path');
      return;
    }
    ...
    try {
      ...
      const defaultCode = renderers.default || renderers[Object.keys(renderers)[0]];
      if (!defaultCode) {
  ...
  const handleInsertFrame = () => {
    ...
    const { name: componentName, path: componentPath } = getComponentInfo(uiComponents.containerComponentPath, 'div');
  ...
  const handleInsertText = () => {
    ...
    const { name: componentName, path: componentPath } = getComponentInfo(uiComponents.textComponentPath, 'span');
  ...
  const handleInsertLink = () => {
    ...
    const { name: componentName, path: componentPath } = getComponentInfo(uiComponents.linkComponentPath, 'a');
  ...
  const handleInsertButton = () => {
    ...
    const { name: componentName, path: componentPath } = getComponentInfo(uiComponents.buttonComponentPath, 'button');
  ...
  const handleInsertImage = () => {
    ...
    const { name: componentName, path: componentPath } = getComponentInfo(uiComponents.imageComponentPath, 'img');
    ...
    if (onOpenInsertPanel) {
      ...
      engine.insertASTElement(
        ...
        meta.filePath,
        componentName,
        { src: IMAGE_PLACEHOLDER, alt: 'Image' },
        componentPath,
      );
    }
  };
  ...
  useEffect(() => {
    if (mode === 'code' || !onResetZoom) {
      console.log('[Toolbar] Reset zoom listener NOT active, mode:', mode, 'onResetZoom:', !!onResetZoom);
      return;
    }
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 h-12 px-2 bg-background rounded-[14px] shadow-[0_2px_4px_rgba(0,0,0,0.15),0_2px_16px_rgba(0,0,0,0.15)] border border-border z-[1000]">
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <IconLayoutGrid className={`w-6 h-6 ${mode === 'board' ? 'text-white' : ''}`} stroke={1.5} />
            </button>
          </TooltipTrigger>
          <TooltipContent>
            <p className="whitespace-nowrap">
              Board <span className="text-xs text-muted-foreground ml-1">1 / Alt+1 / Ctrl+Shift+1</span>
            </p>
          </TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <IconPointer className={`w-6 h-6 ${mode === 'interact' ? 'text-white' : ''}`} stroke={1.5} />
            </button>
          </TooltipTrigger>
          <TooltipContent>
            <p className="whitespace-nowrap">
              Interact <span className="text-xs text-muted-foreground ml-1">2 / Alt+2 / Ctrl+Shift+2</span>
            </p>
          </TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <IconBrush className={`w-6 h-6 ${mode === 'design' ? 'text-white' : ''}`} stroke={1.5} />
            </button>
          </TooltipTrigger>
          <TooltipContent>
            <p className="whitespace-nowrap">
              Design <span className="text-xs text-muted-foreground ml-1">3 / Alt+3 / Ctrl+Shift+3</span>
            </p>
          </TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <IconCode className={`w-6 h-6 ${mode === 'code' ? 'text-white' : ''}`} stroke={1.5} />
            </button>
          </TooltipTrigger>
          <TooltipContent>
            <p className="whitespace-nowrap">
              Code <span className="text-xs text-muted-foreground ml-1">4 / Alt+4 / Ctrl+Shift+4</span>
            </p>
          </TooltipContent>
        </Tooltip>
          ...
          <>
            ...
            {!isReadonly && (
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    Add instance <span className="text-xs text-muted-foreground ml-1">I</span>
                  </p>
                </TooltipContent>
              </Tooltip>
            )}
              ...
              <>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      onClick={() => onBoardToolChange?.(boardTool === 'text' ? 'select' : 'text')}
                      className={`w-8 h-8 rounded-md flex items-center justify-center ${
                        boardTool === 'text' ? 'bg-[#4597F7]' : 'hover:bg-accent'
                      }`}
                    >
                      <IconLetterT className={`w-6 h-6 ${boardTool === 'text' ? 'text-white' : ''}`} stroke={1.5} />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>
                      Text <span className="text-xs text-muted-foreground ml-1">T</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      onClick={() => onBoardToolChange?.(boardTool === 'arrow' ? 'select' : 'arrow')}
                      className={`w-8 h-8 rounded-md flex items-center justify-center ${
                  ...
                  <TooltipContent>
                    <p>
                      Arrow <span className="text-xs text-muted-foreground ml-1">Shift+A</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
            ...
            {!isReadonly && (
              <>
                ...
                >
                  <button type="button" className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-accent">
                  ...
                  {colorPopupOpen && (
                    <div className="absolute bottom-full mb-1 bg-popover rounded-lg shadow-lg border border-border p-2 z-[1001]">
                      <div className="flex gap-1">
                        {['#000000', '#ef4444', '#3b82f6', '#22c55e', '#eab308', '#f97316', '#a855f7'].map((color) => (
                          <button
                            key={color}
                            type="button"
                            onClick={() => onDrawingStyleChange?.({ color })}
                            className={`w-6 h-6 rounded border-2 ${
                              drawingStyle.color === color ? 'border-foreground' : 'border-border'
                            }`}
                      ...
                      <div className="flex flex-col gap-2">
                        {[1, 2, 4, 8].map((width) => (
                          <button
                            key={width}
                            type="button"
                            onClick={() => onDrawingStyleChange?.({ strokeWidth: width })}
                            className={`flex items-center px-3 py-2 rounded hover:bg-accent ${
                              drawingStyle.strokeWidth === width ? 'bg-accent' : ''
                            }`}
                          >
                            <div
                      ...
                      <div className="flex flex-col gap-2">
                        {[16, 20, 28, 36].map((size) => (
                          <button
                            key={size}
                            type="button"
                            onClick={() => onDrawingStyleChange?.({ fontSize: size })}
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <IconSparkles className={`w-6 h-6 ${isAIChatOpen ? 'text-white' : ''}`} stroke={1.5} />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p className="whitespace-nowrap">
                      AI Agent <span className="text-xs text-muted-foreground ml-1">⌘K / ⌘P / ⌘⇧P</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
              </>
          ...
          <>
            ...
            {!isReadonly && (
              <>
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>
                      Frame <span className="text-xs text-muted-foreground ml-1">F</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>
                      Link <span className="text-xs text-muted-foreground ml-1">Shift+L</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>
                      Button <span className="text-xs text-muted-foreground ml-1">B</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>
                      Image <span className="text-xs text-muted-foreground ml-1">⇧I / ⌘⇧K</span>
                    </p>
                  </TooltipContent>
                </Tooltip>
                <Divider />
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <IconMessageCircle className={`w-6 h-6 ${isAddingComment ? 'text-white' : ''}`} stroke={1.5} />
                </button>
              </TooltipTrigger>
              <TooltipContent>
                <p className="whitespace-nowrap">
                  Add Comment <span className="text-xs text-muted-foreground ml-1">C</span>
                </p>
              </TooltipContent>
            </Tooltip>
