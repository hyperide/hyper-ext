...
export function CanvasElementContextMenu({
  ...
  useEffect(() => {
    ...
    const installHandler = () => {
      ...
      iframe.contentDocument.addEventListener('contextmenu', handleIframeContextMenu);
    };
    ...
    return () => {
      ...
      if (iframe?.contentDocument) {
        iframe.contentDocument.removeEventListener('contextmenu', handleIframeContextMenu);
      }
  ...
  useEffect(() => {
    ...
    const handleInstanceOverlayContextMenu = (e: MouseEvent) => {
      ...
      const instanceFrame = eventTarget.closest('[data-instance-frame]') as HTMLElement;
      const instanceBadge = eventTarget.closest('[data-instance-badge]') as HTMLElement;
      const instanceId = instanceFrame?.dataset.instanceFrame || instanceBadge?.dataset.instanceBadge;
    ...
    document.addEventListener('contextmenu', handleInstanceOverlayContextMenu, true);

    return () => {
      document.removeEventListener('contextmenu', handleInstanceOverlayContextMenu, true);
    };
  }, [engine]);
  ...
  const handlePaste = useCallback(async () => {
    if (engine && meta?.filePath) {
      try {
        ...
        const newId = await engine.pasteASTElement(selectedIds[0] || null, meta.filePath, tsxCode);
    ...
    if (engine && meta?.relativeFilePath) {
      ...
      try {
        ...
        if (projectId) {
          ...
          const fileResponse = await authFetch(`/api/read-file?path=${encodeURIComponent(meta.relativeFilePath)}`);
  ...
  const matchesFilter = (text: string) => filter === '' || text.toLowerCase().includes(filterLower);
              ...
              switch (menuType) {
                  ...
                  return (
                    <>
                      {!isReadonly && matchesFilter('edit') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleInstanceEditClick)}>Edit</MenuItem>
                      )}
                      {matchesFilter('copy') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleInstanceCopyClick)}>
                          Copy <span className="ml-auto text-xs text-muted-foreground">⌘C</span>
                        </MenuItem>
                      )}
                      {!isReadonly && matchesFilter('cut') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleInstanceCutClick)}>
                          Cut <span className="ml-auto text-xs text-muted-foreground">⌘X</span>
                        </MenuItem>
                      )}
                      {!isReadonly && matchesFilter('duplicate') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleInstanceDuplicateClick)}>
                          Duplicate <span className="ml-auto text-xs text-muted-foreground">⌘D</span>
                        </MenuItem>
                      )}
                      {!isReadonly && matchesFilter('delete') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleInstanceDeleteClick)}>
                ...
                case 'board-empty':
                  return (
                    !isReadonly &&
                    matchesFilter('paste') && (
                      <MenuItem onClick={() => handleMenuItemClick(handleInstancePasteClick)}>
                ...
                case 'design-element':
                  return (
                    <>
                      ...
                      {(matchesFilter('select parent') || matchesFilter('select child')) && <MenuSeparator />}

                      {matchesFilter('select parent') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleSelectParent)} disabled={isDisabled}>
                          Select Parent <span className="ml-auto text-xs text-muted-foreground">⇧↵</span>
                        </MenuItem>
                      )}
                      {matchesFilter('select child') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleSelectChild)} disabled={isDisabled}>
                          Select Child <span className="ml-auto text-xs text-muted-foreground">↵</span>
                        </MenuItem>
                      )}
                      ...
                      {matchesFilter('copy text') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleCopyText)} disabled={isDisabled}>
                          Copy Text
                        </MenuItem>
                      )}
                      {matchesFilter('copy as html') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleCopyAsHTML)} disabled={isDisabled}>
                          Copy as HTML
                        </MenuItem>
                      )}
                      {!isReadonly && matchesFilter('wrap') && (
                        <MenuItem onClick={() => handleMenuItemClick(handleWrapInDiv)} disabled={isDisabled}>
                          Wrap in div
                        </MenuItem>
                      )}
                    </>
