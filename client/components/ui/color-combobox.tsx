 ...
 */
function findClosestColor(hex: string, system: TokenSystem): { token: string; hex: string } | null {
  if (!hex) return null;
  ...
  return (
    <div className={cn('flex items-center gap-0.5', className)}>
        ...
        >
          <PopoverTrigger asChild>
            ...
            >
              ...
              <span className="text-foreground truncate flex-1 text-left">
                {currentToken ? (tokenSystem === 'tamagui' ? `$${currentToken}` : currentToken) : inputPlaceholder}
              </span>
          ...
          <PopoverContent className="w-[280px] p-0" align="start">
            <Command shouldFilter={false}>
              <CommandInput placeholder="Search colors..." className="h-9" value={search} onValueChange={setSearch} />
                ...
                {isSearching ? (
                  Object.entries(filteredGroups).map(([groupName, options]) => (
                    <CommandGroup
                      key={groupName}
                      heading={
                        groupName === 'special' ? 'Basic' : groupName.charAt(0).toUpperCase() + groupName.slice(1)
                      }
                    >
                      {options.map((option) => (
                        <CommandItem
                      ...
                      .map(([colorName, options]) => (
                        <CommandGroup key={colorName} heading={colorName.charAt(0).toUpperCase() + colorName.slice(1)}>
                          <div
                            className={cn(
                              'grid gap-0.5 p-1',
                              tokenSystem === 'tamagui' ? 'grid-cols-12' : 'grid-cols-11',
                            )}
                          >
                            {options.map((option) => (
                              <button
                                ...
                                title={tokenSystem === 'tamagui' ? `$${option.value}` : option.value}
                                className={cn(
      ...
      <button
        ...
        title={isLinked ? `Unlink from ${tokenSystem} tokens` : `Link to nearest ${tokenSystem} token`}
        className={cn(
