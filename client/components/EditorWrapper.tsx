...
function CanvasEngineSetup({ children }: { children: React.ReactNode }) {
	...
	const engine = useMemo(() => {
		...
		async function loadComponentStructure(data: any) {
			if (data.success && data.componentName) {
				...
				if (!isSameFile) {
					console.log(
						'[Canvas Engine] Different component - clearing instances',
					);
					eng.clearInstances();
					eng.clearHistory();
				} else {
					console.log(
						'[Canvas Engine] Same component - updating structure only',
					);
				}
				...
				if (!isSameFile) {
					...
					if (componentDef) {
						...
						const instanceId = eng.loadInstances(
							data.componentName,
							{},
							null,
							[],
						...
						console.log(
							'[Canvas Engine] Created component instance:',
							instanceId,
							'type:',
							data.componentName,
						);
						...
						console.log(
							'[Canvas Engine] Component not registered (iframe mode):',
							data.componentName,
						);
					}
				}
				...
				if (data.structure && filePath) {
					...
					console.log(
						'[Canvas Engine] New structure has',
					...
					try {
						...
						if (!injectResult.success) {
							console.error(
								'[Canvas Engine] Failed to inject unique IDs:',
								injectResult.error,
							);
						} else {
							console.log(
							...
							if (injectResult.addedCount && injectResult.addedCount > 0) {
								...
								const reParseResponse = await authFetch(
									`/api/parse-component?path=${encodeURIComponent(filePath)}`,
								);
								...
								if (reParseData.success && reParseData.structure) {
									...
									console.log(
										'[Canvas Engine] ✓ Updated AST structure with new IDs',
									);
									// Trigger store update so components react to astStructure change
									eng.events.emit('tree:change', {
										changedIds: [root.id],
				...
				console.error(
					'[Canvas Engine] Component definition not found:',
					data.componentName,
				);
			}
		}
		...
		window.addEventListener(
			'component-loaded',
			handleComponentLoaded as EventListener,
		);
		...
		const handleFilesChanged = () => {
			...
			if (currentFilePath) {
				console.log(
					'[Canvas Engine] Files changed, reloading current component:',
					currentFilePath,
				);
				// Re-parse component to inject missing IDs
				authFetch(
					`/api/parse-component?path=${encodeURIComponent(currentFilePath)}`,
				)
					.then((res) => res.json())
					.then((data) => {
						if (data.success) {
		...
		window.addEventListener(
			'components-updated',
			handleFilesChanged as EventListener,
		);
	...
	useEffect(() => {
			...
			authFetch('/api/get-component-definitions')
				...
				.then((data) => {
					if (data.success && data.definitions && data.definitions.length > 0) {
						...
						if (notifyListeners) {
							console.log(
								'[Canvas Engine] Dispatching reload-component-definitions event',
							);
		...
		const handleAIGenerationComplete = () => {
			console.log(
				'[Canvas Engine] AI generation complete - reloading definitions',
			);
	...
	return (
		<CanvasEngineProvider engine={engine}>{children}</CanvasEngineProvider>
	);
}
