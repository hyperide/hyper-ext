...
export default function IframeCanvas({
	...
	const [canvasComposition, setCanvasComposition] =
		useState<CanvasComposition | null>(null);
		...
		try {
			...
			for (const pattern of errorPatterns) {
				if (pattern.test(content)) {
					...
					return {
						hasError: true,
						errorMessage: proxyErrorMatch
							? proxyErrorMatch[1].trim()
							: undefined,
					};
				}
			}
			...
			console.log(
				'[IframeCanvas] Cannot access iframe content, might be error page',
			);
	...
	useEffect(() => {
		(window as any).__CANVAS_INSTANCES__ = canvasComposition?.instances ?? {};
	}, [canvasComposition]);
	...
	useEffect(() => {
		...
		const loadComposition = async () => {
			...
			try {
				...
				console.error(
					'[IframeCanvas] Failed to load composition from server:',
					error,
				);
			}
			...
			const instanceKeys = composition
				? Object.keys(composition.instances)
				: [];
			const isMultiMode =
				instanceKeys.length > 1 ||
				(instanceKeys.length === 1 && instanceKeys[0] !== 'default');
		...
		const handleCanvasChanged = async () => {
			...
			try {
				...
				if (response.ok) {
					...
					console.log(
						'[IframeCanvas] Composition reloaded after canvasCompositionChanged',
					);
				}
			} catch (error) {
				...
				if (errorCheck.hasError) {
					...
					retryTimeoutRef.current = setTimeout(() => {
						if (retryCount < 10) {
							...
							setError(
								'Failed to connect to project. Please check if the project is running.',
							);
							setLoading(false);
						}
					}, delay);
	...
	useEffect(() => {
		...
		const handleClick = (e: MouseEvent) => {
			...
			if (isAddingComment && onAddComment) {
				...
				const instanceId =
					instanceElement?.dataset.canvasInstanceId || activeInstanceId || null;
			...
			if (mode === 'design' || mode === 'interact') {
				...
				if (activeInstanceId) {
					const instanceElement = element.closest(
						'[data-canvas-instance-id]',
					) as HTMLElement;
				...
				if (mode === 'design' && onElementClick) {
					...
					const itemIndex = uniqId
						? getItemIndex(element, uniqId, doc, activeInstanceId)
						: null;
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (isMod && canvasHotkeys.includes(e.key.toLowerCase())) {
				console.log(
					'[IframeCanvas] ⏭️ Skipping canvas hotkey, letting it reach handleIframeKeydown:',
					e.key,
				);
				return;
			}
		...
		if (iframeWindow) {
			console.log(
				'[IframeCanvas] ✅ Installing keyboard listeners on iframe window',
			);
			...
			console.warn(
				'[IframeCanvas] ⚠️ iframe.contentWindow is null, cannot install keyboard listeners',
			);
		}
		...
		return () => {
			...
			if (iframeWindow) {
				console.log(
					'[IframeCanvas] Removing keyboard listeners from iframe window',
				);
	...
	useEffect(() => {
		...
		const instances = iframe.contentDocument.querySelectorAll(
			'[data-canvas-instance-id]',
		);
	...
	useEffect(() => {
		...
		const checkForRuntimeError = (): RuntimeError | null => {
			...
			const nextjsOverlay = nextjsShadow?.querySelector(
			...
			if (nextjsOverlay && nextjsShadow) {
				...
				const errorLabel = nextjsShadow.querySelector(
					'#nextjs__container_errors_label',
				);
				const errorType = errorLabel?.textContent?.trim() || 'Error';

				// Extract error message
				const errorDesc = nextjsShadow.querySelector(
					'#nextjs__container_errors_desc',
				);
				const errorMessage = errorDesc?.textContent?.trim() || 'Unknown error';

				// Extract file and line from codeframe
				const codeframeLink = nextjsShadow.querySelector(
					'[data-nextjs-codeframe] [data-text]',
				);
				const fileLine = codeframeLink?.textContent?.trim() || '';

				// Extract codeframe
				const codeframePre = nextjsShadow.querySelector(
			...
			if (viteOverlay?.shadowRoot) {
				...
				const codeframe =
					frameEl?.textContent?.trim().slice(0, 500) || undefined;
			...
			if (bunHmr?.shadowRoot) {
				...
				if (errorContent) {
					...
					const codeframe =
						stackTrace?.textContent?.trim().slice(0, 500) || undefined;
	...
	const iframeSize = useMemo(() => {
		console.log('[IframeCanvas] iframeSize calc:', {
			...
			instanceCount: canvasComposition?.instances
				? Object.keys(canvasComposition.instances).length
				: 0,
		});
		...
		if (
			!canvasComposition?.instances ||
			Object.keys(canvasComposition.instances).length === 0
		) {
			console.log(
				'[IframeCanvas] iframeSize: no instances, returning default 10000x10000',
			);
