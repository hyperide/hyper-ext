...
export default function AISettings() {
	...
	const {
		...
		async () => {
			...
			const response = await authFetch(
				`/api/ai-config?workspaceId=${currentWorkspace.id}`,
			);
			if (!response.ok) {
			...
			<Card>
				<CardContent className="py-12 text-center">
					<p className="text-destructive">
						{configError || 'Failed to load AI configuration'}
					</p>
				</CardContent>
			</Card>
		);
	...
	const hasExistingProxyKey =
		isProxyProvider && config.apiKey && config.apiKey.length > 0;
	const hasExistingOpenCodeKey =
		isOpenCodeProvider && config.apiKey && config.apiKey.length > 0;
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Configure the AI model used for code generation. Default is GLM-4.6
					via Z.ai.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<form onSubmit={handleSave} className="space-y-6">
					<div className="space-y-2">
						...
						>
							...
							<option value="proxy">
								Proxy (Gemini, DeepSeek, Mistral, Groq)
							</option>
							<option value="opencode">
								OpenCode (Gemini, DeepSeek, Qwen)
							</option>
						</select>
					</div>
					...
					{isProxyProvider ? (
						<>
							...
							<div className="space-y-2">
								...
								>
									...
									{Object.entries(PROXY_MODELS_GROUPED).map(
										([publisher, models]) => (
						...
						<>
							...
							<div className="space-y-2">
								...
								>
									<option value="">Select a model...</option>
									{Object.entries(OPENCODE_MODELS_GROUPED).map(
										([publisher, models]) => (
											<optgroup key={publisher} label={publisher}>
												{models.map((model) => (
													<option key={model.id} value={model.id}>
								...
								<p className="text-xs text-muted-foreground">
									OpenCode server will start automatically when you use the AI
									agent
								</p>
							</div>
						...
						<>
							...
							<div className="space-y-2">
								...
								<p className="text-sm text-muted-foreground">
									Examples: glm-4.7, glm-4.6, glm-4
								</p>
							</div>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setConfig({ ...config, baseURL: e.target.value || null })
									}
						...
						<>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) => setApiKey(e.target.value)}
									placeholder={
										config.apiKey ? `Current: ${config.apiKey}` : 'sk-ant-...'
									}
								/>
								<p className="text-sm text-muted-foreground">
									{config.apiKey
						...
						<>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) => setApiKey(e.target.value)}
									placeholder={
										config.apiKey ? `Current: ${config.apiKey}` : 'sk-...'
									}
								/>
								<p className="text-sm text-muted-foreground">
									{config.apiKey
					...
					<div className="space-y-2 pt-4 border-t">
						...
						<p className="text-xs text-muted-foreground">
							...
							<code className="bg-muted px-1 rounded">{'{diff}'}</code> for
							changes and{' '}
							<code className="bg-muted px-1 rounded">{'{examples}'}</code> for
							...
							<Input
								...
								placeholder={
									config.braveSearchApiKey
										? `Current: ${config.braveSearchApiKey}`
