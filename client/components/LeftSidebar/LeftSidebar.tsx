...
export default function LeftSidebar({
	...
	const saasComponentMeta = engine ? useComponentMeta() : null;
	const meta = saasComponentMeta?.meta ?? null;
	const isPushPopoverOpen = engine ? useGitStore().isPushPopoverOpen : false;
	...
	const currentComponentPath = engine
		? meta?.relativeFilePath
		: componentNav.activePath ?? undefined;

	const { testGroups, isLoading: isLoadingTests, reload: reloadTests } =
		useTestGroups(currentComponentPath, meta?.projectId);

	const elementsTree = useElementsTree(meta?.componentName);

	const { selectedIds, hoveredId: selectionHoveredId, handleSelect, handleHover } =
	...
	return (
		...
		>
			...
			>
				...
				<ResizeHandle onPointerUp={() => { if (isPushPopoverOpen) handleResizeEnd(['source-control', 'pages']); }} />
