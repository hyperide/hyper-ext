/**
 * Tailwind CSS value constants for use in UI pickers
...
/**
 * Get hex value for a Tailwind color class (e.g., 'blue-500' => '#3b82f6')
 */
export function getColorHex(colorClass: string): string | null {
  const parts = colorClass.split('-');
  if (parts.length < 2) {
    // Handle special colors: white, black, etc.
    const special = TAILWIND_COLORS[colorClass as keyof typeof TAILWIND_COLORS];
    return typeof special === 'string' ? special : null;
  }

  const [colorName, shade] = parts;
  const colorData = TAILWIND_COLORS[colorName as keyof typeof TAILWIND_COLORS];

  if (!colorData || typeof colorData === 'string') return null;

  // Check if shade exists in colorData
  if (shade in colorData) {
    return (colorData as Record<string, string>)[shade];
  }
