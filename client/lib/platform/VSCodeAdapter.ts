...
export function createVSCodeSSEAdapter(): SSEAdapter {
  ...
  return {
    subscribe(
      ...
      const handler = (message: PlatformMessage) => {
        if (message.type === 'sse:message' && message.subscriptionId === subscriptionId) {
          callbacks.onMessage(message.event, message.data);
        } else if (message.type === 'sse:error' && message.subscriptionId === subscriptionId) {
          callbacks.onError?.(message.error);
        } else if (message.type === 'sse:connected' && message.subscriptionId === subscriptionId) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const handler = (message: PlatformMessage) => {
          if (message.type === 'api:response' && message.requestId === requestId) {
            ...
            resolve(response);
          } else if (message.type === 'api:error' && message.requestId === requestId) {
            messageHandlers.delete(handler);
