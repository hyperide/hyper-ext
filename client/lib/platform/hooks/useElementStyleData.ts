...
import { getPreviewIframe, buildElementSelector } from '@/lib/dom-utils';
...
): Partial<ParsedStyles> | undefined {
...
export function classNameToStyles(className: string): ParsedStyles {
	...
	const flexDirection: 'row' | 'column' | undefined =
		parsed.flexDirection === 'column'
			? 'column'
			: parsed.flexDirection === 'row'
				? 'row'
				: undefined;
 ...
 */
export function useElementStyleData(
	options: UseElementStyleDataOptions,
): ElementStyleData {
	...
	useEffect(() => {
		...
		if (engine && styleAdapter) {
			...
			if (root.metadata?.astStructure) {
				astNode = findNodeById(root.metadata.astStructure, elementId);
			}
			...
			if (!astNode) {
				for (const childId of root.children || []) {
					...
					if (inst?.metadata?.astStructure) {
						astNode = findNodeById(inst.metadata.astStructure, elementId);
						if (astNode) break;
					}
				}
			}
			...
			if (astNode.childrenType !== 'jsx') {
				textContent = astNode.childrenType
					? (astNode.props?.children || '')
					: domTextContent;
			}
