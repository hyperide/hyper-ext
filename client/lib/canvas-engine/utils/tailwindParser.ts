...
function parseColors(
  ...
  ParsedTailwindStyles,
  "backgroundColor" | "backgroundImage" | "textColor" | "borderColor"
> {
  const result: Pick<
    ParsedTailwindStyles,
    "backgroundColor" | "backgroundImage" | "textColor" | "borderColor"
  ...
  for (const cls of classes) {
    ...
    if (cls === "border") {
      ...
      if (
        arbValue &&
        !arbValue.includes("#") &&
        (arbValue.includes("px") ||
          arbValue.includes("rem") ||
          arbValue.includes("em"))
      ) {
...
function parseOverflow(
  classes: string[],
): Pick<ParsedTailwindStyles, "overflow"> {
...
function parseOpacity(
  classes: string[],
): Pick<ParsedTailwindStyles, "opacity"> {
  ...
  for (const cls of classes) {
    if (cls === "shadow") {
      ...
      if (match) {
        ...
        const justColorMatch = shadowValue.match(
        ...
        if (justColorMatch) {
          ...
          if (rgbaMatch) {
            ...
            // Convert to hex
            const hex = `#${Number.parseInt(r).toString(16).padStart(2, "0")}${Number.parseInt(g).toString(16).padStart(2, "0")}${Number.parseInt(b).toString(16).padStart(2, "0")}`;
            result.shadowColor = hex;
            result.shadowOpacity = a
              ? `${Math.round(Number.parseFloat(a) * 100)}`
              : "100";
          } else {
            // Try to extract hex color (e.g., #c90303)
            const hexMatch = shadowValue.match(
 ...
 * // }
 */
export function mapPropertiesToTailwindClasses(
  domClasses: string,
  ...
  for (const [shadowKey, classes] of Object.entries(shadowClasses)) {
    if (classes.length > 0) {
      const prefix =
        shadowKey === "base" ? "" : `${modifierToCamelCase(shadowKey)}.`;
 ...
 */
export function getClassNameFromNode(node: any): string {
