...
export function createAnnotationStore(
	...
	function serverAdd(annotation: AnnotationElement) {
		authFetch(
			`/api/canvas-composition/${projectId}/annotation`,
			{
				method: 'POST',
	...
	function serverUpdate(id: string, annotation: AnnotationElement) {
		authFetch(
			`/api/canvas-composition/${projectId}/annotation/${encodeURIComponent(id)}`,
			{
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ componentPath, annotation }),
	...
	function serverBatchDelete(ids: string[]) {
		authFetch(
			`/api/canvas-composition/${projectId}/annotations/batch-delete`,
			{
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
			...
			console.error('[AnnotationStore] batch-delete failed:', err),
		);
	}
 ...
 */
export function useAnnotationStoreSnapshot(
	store: AnnotationStoreApi,
): AnnotationElement[] {
