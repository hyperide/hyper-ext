...
import { type Layout, type PanelImperativeHandle, useGroupRef } from 'react-resizable-panels';
...
export function useSidebarPanelLayout({
  ...
  const { hasRawPagesContent, hasRawComponentsContent, hasElementsContent, hasTestsContent } = contentFlags;
  ...
  const [elementsTreeCollapsed, setElementsTreeCollapsed] = useState(
    () => defaultLayout?.['elements-tree'] !== undefined && defaultLayout['elements-tree'] <= 24,
  );
  ...
  const handleUserToggle = useCallback(
    (panelId: string, animatedToggle: () => void, panelRef: RefObject<PanelImperativeHandle | null>) => {
  ...
  useEffect(() => {
    ...
    const toExpand = panelConfigs.filter((p) => p.hasContent && !userManuallyCollapsed.current.has(p.id));
    ...
    if (toExpand.length > 0) {
      requestAnimationFrame(() => {
        ...
        const collapsedPercent =
          toCollapse.length > 0 && toCollapse[0].ref.current ? toCollapse[0].ref.current.getSize().asPercentage : 2;
        ...
        for (const p of manuallyCollapsedNotEmpty) {
          const pctNow = p.ref.current?.getSize().asPercentage ?? collapsedPercent;
